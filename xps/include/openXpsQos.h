 ...
 * (L2/L3/MPLS) supported Each profile provides the ability
 * to configure PCP/DEI to traffic class mapping
 * independently
 *
 * \param [in] devId Device id
 ...
 * There are 8 QOS Profiles for each type of QOS marking
 * (L2/L3/MPLS) supported. Each profile provides the ability
 * to configure PCP/DEI to traffic class mapping * independently
 *
 * \param [in] devId
 * \param [in] profile
 * \param [in] pcpVal
 ...
 * There are 8 QOS profiles for each type of QOS marking
 * (L2/L3/MPLS) supported. Each profile provides the ability
 * to configure DSCP to traffic class mapping ndependently
 *
 * \param [in] devId Device id
 * \param [in] profile QOS Profile
 ...
 *
 * There are 8 QOS Profiles for each type of QOS marking
 * (L2/L3/MPLS) supported. Each profile provides the
 ...
 * \return XP_STATUS
 */
XP_STATUS xpsQosFcSetPfcPriority(xpsDevice_t devId, uint32_t portNum,
...
/**
 * \brief API to read the Drop Length counter for a Queue
 *        associated with a given port
 *
 * Wrap indicates if the counter has wrapped around since the
 * last read
...
/**
 * \brief API to map reason code to CPU queue
 *
 * \param [in] devId
 * \param [in] cpuType
 * \param [in] reasonCode
 * \param [in] cpuQueueNum
 *
 * \return XP_STATUS
 */
XP_STATUS xpsQosCpuScAddReasonCodeToCpuQueueMap(xpsDevice_t devId,
                                                xpCpuTypes_e cpuType, uint32_t reasonCode, uint32_t cpuQueueNum);

/**
 * \brief API to Control PCP/DEI Remark Enable/Disable Globally
 *
 * \param [in] devId
 * \param [in] enable
 *
...
/**
 * \brief API to Control DSCP Remark Enable/Disable Globally
 *
 * \param [in] devId
 * \param [in] enable
 *
