...
if sys.version_info[0] == 2:
    xpPyparsing.ParserElement.enablePackrat()

"""
Packrat is causing Python3 errors that I don't understand.

> /usr/local/Cellar/python3/3.2/lib/python3.2/site-packages/xpPyparsing-1.5.6-py3.2.egg/xpPyparsing.py(999)scanString()
...
def options(option_list, arg_desc="arg"):
    ...
    for opt in option_list:
        options_defined.append(xpPyparsing.Literal(opt.get_opt_string()))
...
xpPyparsing.ParserElement.setDefaultWhitespaceChars(' \t')
...
class Cmd(cmd.Cmd):
    ...
    legalChars = u'!#$%.:?@_' + xpPyparsing.alphanums + xpPyparsing.alphas8bit
    ...
    def do_help(self, arg):
        if arg:
            ...
            if funcname:
                ...
                if '<' in fn.__doc__  and '>' in fn.__doc__:
                    enum_name = fn.__doc__.split('<')[1].split('>')[0]
                    enum_name = enum_name.split()
                    for enumName in enum_name:
                        enumDict.printEnumDict(enumName)
        else:
    ...
    def do_prompt(self, arg):
        dirs = self.prompt
        print('Current Prompt is : %s' % dirs)
    ...
    prefixParser = xpPyparsing.Empty()
    commentGrammars = xpPyparsing.Or([xpPyparsing.pythonStyleComment, xpPyparsing.cStyleComment])
    commentGrammars.addParseAction(lambda x: '')
    commentInProgress  = xpPyparsing.Literal('/*') + xpPyparsing.SkipTo(
        xpPyparsing.stringEnd ^ '*/')
    terminators = [';']
    ...
    def _init_parser(self):
        ...
        - statement: ['what', 'if "quoted strings /* seem to " start comments?']
          ...
          - command: what
        '''
        #outputParser = (xpPyparsing.Literal('>>') | (xpPyparsing.WordStart() + '>') | xpPyparsing.Regex('[^=]>'))('output')
        outputParser = (xpPyparsing.Literal(self.redirector *2) | \
                       (xpPyparsing.WordStart() + self.redirector) | \
                        xpPyparsing.Regex('[^=]' + self.redirector))('output')
        
        terminatorParser = xpPyparsing.Or([(hasattr(t, 'parseString') and t) or xpPyparsing.Literal(t) for t in self.terminators])('terminator')
        stringEnd = xpPyparsing.stringEnd ^ '\nEOF'
        self.multilineCommand = xpPyparsing.Or([xpPyparsing.Keyword(c, caseless=self.case_insensitive) for c in self.multilineCommands])('multilineCommand')
        oneLineCommand = (~self.multilineCommand + xpPyparsing.Word(self.legalChars))('command')
        pipe = xpPyparsing.Keyword('|', identChars='|')
        self.commentGrammars.ignore(xpPyparsing.quotedString).setParseAction(lambda x: '')
        doNotParse = self.commentGrammars | self.commentInProgress | xpPyparsing.quotedString
        afterElements = \
            xpPyparsing.Optional(pipe + xpPyparsing.SkipTo(outputParser ^ stringEnd, ignore=doNotParse)('pipeTo')) + \
            xpPyparsing.Optional(outputParser + xpPyparsing.SkipTo(stringEnd, ignore=doNotParse).setParseAction(lambda x: x[0].strip())('outputTo'))
        ...
        if self.blankLinesAllowed:
            self.blankLineTerminationParser = xpPyparsing.NoMatch
        else:
            self.blankLineTerminator = (xpPyparsing.lineEnd + xpPyparsing.lineEnd)('terminator')
            self.blankLineTerminator.setResultsName('terminator')
            self.blankLineTerminationParser = ((self.multilineCommand ^ oneLineCommand) + xpPyparsing.SkipTo(self.blankLineTerminator, ignore=doNotParse).setParseAction(lambda x: x[0].strip())('args') + self.blankLineTerminator)('statement')
        self.multilineParser = (((self.multilineCommand ^ oneLineCommand) + xpPyparsing.SkipTo(terminatorParser, ignore=doNotParse).setParseAction(lambda x: x[0].strip())('args') + terminatorParser)('statement') +
                                xpPyparsing.SkipTo(outputParser ^ pipe ^ stringEnd, ignore=doNotParse).setParseAction(lambda x: x[0].strip())('suffix') + afterElements)
        self.multilineParser.ignore(self.commentInProgress)
        self.singleLineParser = ((oneLineCommand + xpPyparsing.SkipTo(terminatorParser ^ stringEnd ^ pipe ^ outputParser, ignore=doNotParse).setParseAction(lambda x:x[0].strip())('args'))('statement') +
                                 xpPyparsing.Optional(terminatorParser) + afterElements)
        ...
        self.parser = self.prefixParser + (
            ...
            self.multilineCommand + xpPyparsing.SkipTo(stringEnd, ignore=doNotParse)
            )
        self.parser.ignore(self.commentGrammars)
        
        inputMark = xpPyparsing.Literal('<')
        inputMark.setParseAction(lambda x: '')
        fileName = xpPyparsing.Word(self.legalChars + '/\\')
        inputFrom = fileName('inputFrom')
        inputFrom.setParseAction(replace_with_file_contents)
        # a not-entirely-satisfactory way of distinguishing < as in "import from" from <
        # as in "lesser than"
        self.inputParser = inputMark + xpPyparsing.Optional(inputFrom) + xpPyparsing.Optional('>') + \
                           xpPyparsing.Optional(fileName) + (xpPyparsing.stringEnd | '|')
        ...
        try:
            try:
                ...
                try:
                    ...
                    #Added this to store return value from the command                      
                    if xpShellGlobals.userDefVar is not '':
                        xpShellGlobals.varDict[xpShellGlobals.userDefVar] = xpShellGlobals.cmdRetVal
    ...
    def complete_statement(self, line):
        ...
        if (not line) or (
            not xpPyparsing.Or(self.commentGrammars).
    ...
    def pseudo_raw_input(self, prompt):
        ...
        if self.use_rawinput:
            #print('GETS Raw Input')
            try:
                if filelen > 0:
                    line = filelist[flen]
                    flen = flen+1
                    filelen = filelen - 1
                    #print('filelen = %d - %d' % filelen, flen)
                else:
            ...
            if loadcmd == 1:
                ...
                #print('GETS Input from STDIN')
                self.stdout.write(prompt)
                self.stdout.flush()

                if filen >= len(fileopen):
                    line = 0
                else:
                    line = fileopen[filen]
                filen = filen + 1
        ...
        if savefd != 'None' and line != 'restore' and line != 'save stop' and 'load_config' not in line and line != 'save_config stop' and line != 'save_running_config stop':
            savefd.write(line+'\n')

        #Added to store and reuse return value form xpShell commands
        if ('>' in line):
            xpShellGlobals.userDefVar = line.split('>')[1].strip()        
            line = line.split('>')[0].strip()   
        else:
            xpShellGlobals.userDefVar = '' 

        if ('$' in line):
            #lookup for given variable name in stored variable dictionary
            varList = line.split('$')
            index = 1
            tmpLine = line
            while (index < len(varList)):
                varGiven = tmpLine.split('$')[index].strip().split(' ')[0]
                if varGiven in xpShellGlobals.varDict.keys():
                    line = line.replace('$'+varGiven,str(xpShellGlobals.varDict[varGiven]))
                index += 1
        cdir = 0

        slist = ['cd', 'eof', 'help', 'exit', 'nop', 'pwd', 'prompt', 'run', 'shortcuts',
                 'clear', 'exec', 'home', 'ls', 'pause', 'py', 'shell', '?', 'quit']
        ...
        if line in 'back':
            ...
            for x in slist :
                tx = x + ' '
                rx = ' ' + x
                if tx in line or rx in line or line in x:
                    cdir = 1
                #print('x=%s, line=%s, rx=%s, tx=%s, cdir=%d' % (x, line, rx, tx, cdir))
        ...
        linearg = line.split(" ", 1)
        nline = linearg[0].replace("_","")

        if 'xps' in self.prompt and nline.islower() == False:
            nline = ''
            for x in line:
                if x.isupper():
                    nline = nline + '_' + x.lower()
                else:
                    nline = nline + x
            nline = nline.replace("xps_", "")
            line = nline

        if (xpShellGlobals.sessionFlag == 2) and (cdir == 0):
    ...
    def do_remote(self, arg):
        ...
        xpConfigPipeRd = '/tmp/npiper' + userName + str(0)
        xpConfigPipeWr = '/tmp/npipew' + userName + str(xpShellGlobals.sessionId)

        #print('%s : %s' % (xpConfigPipeRd, xpConfigPipeWr))

        if (os.path.exists(xpConfigPipeRd)) and (os.path.exists(xpConfigPipeWr)):
            
            if os.path.exists(xpShellGlobals.xpConfigFileRd):
                os.remove(xpShellGlobals.xpConfigFileRd)
            ...
            args = arg.replace('remote', '')

            if ('load_config ' in args):
                args = args.replace('load_config ', '')
                copyfile(args, xpShellGlobals.xpConfigFileRd)
            else:
                args = args + '\n'
                wp = open(xpShellGlobals.xpConfigFileRd, 'w+')
                dirs = self.prompt
                dirs = dirs.replace("(","")
                dirs = dirs.replace(")","")
                dirs = dirs.replace(":","/")
            ...
            #indicate to the remote xpShell
            wp = open(xpConfigPipeRd, 'w+')
            if 'relaunch' in arg:
                wp.write('9'+'\n')
                wp.flush()
                wp.close()
            else:
                wp.write(str(xpShellGlobals.sessionId)+'\n')
                #print("Sending sessionId %s" % (str(xpShellGlobals.sessionId)))
                wp.flush()
                wp.close()
                wp = os.open(xpConfigPipeWr, os.O_RDONLY | os.O_NONBLOCK)
                timeout = 0
                delay = 'sleep 1'
                while True:
                    var = os.read(wp, 50)
                    os.system(delay)
                    timeout += 1
                    if var or timeout > 20:
                        break
                os.close(wp)
                if timeout > 20:
                    print('Error:Output timeout, probabely the App is not started OR abruptly terminated.\nPlease retry after sometime or check the App is running.')
                    return
            #print('Now %s' % (xpShellGlobals.xpConfigFileWr))
            if os.path.exists(xpShellGlobals.xpConfigFileWr):
                if os.stat(xpShellGlobals.xpConfigFileWr).st_size > 0:
                    with open(xpShellGlobals.xpConfigFileWr) as f:
                        for line in f:
                            if 'Launching xpShell' in line:
                                continue
                            if 'Loading Configuration file' in line:
                                continue
                            if 'xpShell' in line and 'prompt' in line:
                                continue
                            if 'xpShell' in line and 'back' in line:
                                continue
                            if 'xpShell Exit' in line:
                                continue
                            if 'xpShell' in line:
                                continue
                            if 'Current Prompt is' in line:
                                rline = line.replace('Current Prompt is : ','')
                                rline = rline.strip(' \t\n\r')
                                nline = rline.strip(self.prompt)
                                nline = nline.strip(')')
                                nline = nline.strip(':')
                                if nline != '' and self.prompt != rline:
                                    nline = self.convert(nline)
                                    #print('%s - %s' % (nline, self.prompt))
                                    print('Error, please use \'cd %s\' command to change directory' % (nline))
                                continue
                            print line,
                    ...
                    if not 'relaunch' in arg:
                        print('Error no output, probably primary xpShell is still running. Kill it with exit 2')
            else:
                if not 'relaunch' in arg:
                    print('Error no output, probably primary xpShell is still running. Kill it with exit 2')
        else:
            print('Error, primary xpShell app is not started')
    ...
    def _cmdloop(self, intro=None):
        ...
        if os.path.exists(rdrfile) == True:
            self.loader(rdrfile)
            self.echo = True
        else:
            self.revert(rdrfile)
    ...
    def do_dump(self, arg):
        '''
        Prints the local shell variables
        '''
        if arg:
            print(xpShellGlobals.varDict[arg])
        else:
            print("{}".format(xpShellGlobals.varDict))
    ...
    def do_cd(self,clear):
	print 'Just type valid command to go in to its sub-commands'
	print 'use <back> if you want to go back to parent command'

    def do_exec(self, arg):
        if arg == '':
            print('Invalid input, enter the command to execute')
        else:
            itgt = 'sdk.'
            arg = itgt + arg
            print('arg = %s\n' % arg)
    ...
    #/*****************************************************************************************************/
    def def_back(self, arg):
        print 'Invalid input, check help <command> or ? for usage'
    ...
    #/*****************************************************************************************************/
    def do_nop(self, arg):
        #'No operation. Can be used for comments'
	args = 0

    def history(self, arg, opts):
    ...
    def edit(self, arg):
        ...
        edit [N]: edit numbered command from history
        edit [filename]: edit specified file name
    ...
    saveparser = (xpPyparsing.Optional(xpPyparsing.Word(xpPyparsing.nums)^'*')("idx") + 
                  xpPyparsing.Optional(xpPyparsing.Word(legalChars + '/\\'))("fname") +
                  xpPyparsing.stringEnd)    
    ...
    def save(self, arg):
        ...
        if arg == 'stop':
            if savefd != 'None':
                print('Stopped save configuration')
                savefd.close()
                savefd = 'None'
        else:
            savefd = open(arg, 'a+')

    def filter_runningconfig(self, tmpFile, outFile):
        excludeCmd = []                           #commands that starts with ~
        prefixCmd  = []                           #commands that ends with *
        fixedCmd   = []                           #fixed commands
        notRemove  = 0
        refFile = './cli/ReferenceFile.txt'       #contains filteration commands

        with open(tmpFile) as tmpFd:
            tmpContent = tmpFd.readlines()

        with open(refFile) as refFd:
            refContent = refFd.readlines()

        #group similar types of commands in seperate list
        for item in refContent:
            item = item.rstrip().lstrip()
            if item.startswith('~'):
                excludeCmd.append(item)
            elif (item.endswith('*\n') or item.endswith('*')):
                prefixCmd.append(item)
            elif((item != '\n') and (item != '') and (not item.startswith('#'))):
                if(item[-1] != '\n'):
                    item+='\n'
                fixedCmd.append(item)

        excludeCmd = [i.replace('~', '').replace('\n','') for i in excludeCmd]
        prefixCmd  = [i.replace('*', '').replace('\n','') for i in prefixCmd]

        for item in tmpContent[:]:
            #Remove blank lines
            if(item == '\n'):
                tmpContent.remove(item)
                continue
            #Remove prefixed commands
            for prefix in prefixCmd:
                if(item.startswith(prefix)):
                    notRemove = 0                 #initial item value
                    for excludeItem in excludeCmd:
                        if(item.startswith(excludeItem)):
                            notRemove = 1         #item found in exclude commands
                            break
                    if(notRemove == 0):
                        tmpContent.remove(item)
                        notRemove = 2             #item has been removed
                    break
            #Remove fixed commands
            if(notRemove == 2):
                notRemove = 0
                continue
            notRemove = 0
            for fixCmd in fixedCmd:
                if(item == fixCmd):
                    tmpContent.remove(item)
                    break
        ...
        #write filtered configuration to output file
        tmpContentStr = ''.join(tmpContent)
        fd = open(outFile,"w")
        fd.write(tmpContentStr)
        fd.close()

        #remove input file (temporary file)
        os.remove(tmpFile)

    def save_runningConfig(self, arg):
        global savefd
        args = arg.split()
        filemode = 'a+'
        if (args[1] == '1'):
        ...
        if args[0] == 'stop':
            if savefd != 'None':
                print('Stop saving running configuration')
                savefd.close()
                savefd = 'None'
                Cmd.filter_runningconfig(self, args[2]+'.tmp', args[2])
        else:
            if(filemode == 'a+'):
                if(os.path.isfile(args[2])):
                    os.system('cp '+ args[2] + (' ' + args[2] + '.tmp'))
            savefd = open(args[2] + '.tmp', filemode)
    ...
    def relative_load(self, arg=None):
        '''
    ...
    def loader(self, arg=None):
        ...
        if os.path.exists(targetname) == True:
            rdrfile = targetname
            if standin == '':
                #print('Saving state %s' % standin)
                standin = self.stdin
                #print('Saving state 1 = %s' % standin)
                grawin = self.use_rawinput
                if fileopen == '':
                    filep = open(targetname, 'r')
                    fileopen = filep.readlines()
                    fileopen.append('prompt')
                    for i in range(0, 4):
                        fileopen.append('back\n')
                    if (addexit == 1):
                        fileopen.append('exit 0\n')
                        fileopen.append('exit 0\n')
                        fileopen.append('exit 0\n')
                        addexit = 0
                    fileopen.append('restore\n')
                    #print('fileopen = %s' % fileopen)
                    filep.close()
                    filen = 0
            grawin = self.use_rawinput
            #print('Redirecting state')
            self.use_rawinput = False
            self.echo = True
        else:
            print('File %s not found' % targetname)
            self.revert(rdrfile)
    ...
    def revert(self, arg=None):
        global grawin
        global rdrfile
        global standin

        if standin != '':
            #print('Restoring state %s' % standin)
            #self.lastcmd = ''
            self.stdin = standin
    ...
    def restored(self, arg=None):
        ...
        fileopen = ''
        filen = 0
        #print('Restore done %s' % self.stdin)
        rdrfile = 'invalid file'    
...
class Cmd2TestCase(unittest.TestCase):
    ...
    regexPattern = xpPyparsing.QuotedString(quoteChar=r'/', escChar='\\', multiline=True, unquoteResults=True)
    regexPattern.ignore(xpPyparsing.cStyleComment)
    notRegexPattern = xpPyparsing.Word(xpPyparsing.printables)
