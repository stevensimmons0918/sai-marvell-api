    ...
    if [ "`uname`" == "FreeBSD" ]; then
        ...
        case $CPU_SUBDIR in
            ...
            ARM926EJ)
                ...
                ;;
            ARMv7)
                CALC_UTILITY=arm-marvell-linux-gnueabi-size
                ;;
            ARMv7be)
                CALC_UTILITY=armeb-marvell-linux-gnueabi-size
                ;;
            AARCH64v8)
                CALC_UTILITY=aarch64-marvell-linux-gnu-size
                ;;
            AARCH64v8be)
                CALC_UTILITY=aarch64_be-marvell-linux-gnu-size
                ;;
            i386)
                CALC_UTILITY=size
                ;;
            *)
                echo "Wrong CPU_SUBDIR - ${CPU_SUBDIR}, use size"
                CALC_UTILITY=size
                ;;
        esac
    fi
...
####################################################################
    ...
    do
        ...
        #
        #  count code section size for mainPxDrv libs
        #
        LIB_PATH="${CPSS_LIB_ROOT_PATH}/mainPxDrv/libs/${CPU_SUBDIR}"

        ## remove libs of Pp because they are in mainPxDrv.lib 
        if [ -f "${CPSS_LIB_ROOT_PATH}/mainPxDrv/libs/${CPU_SUBDIR}/mainPxDrv.lib" ]
        then
            rm -f ${CPSS_LIB_ROOT_PATH}/mainPxDrv/libs/${CPU_SUBDIR}/cpss*.lib
            rm -f ${CPSS_LIB_ROOT_PATH}/mainPxDrv/libs/${CPU_SUBDIR}/mainPxDrv.o
        fi
        SizeCount_Function $CPU_SUBDIR $LIB_PATH $WORKING_DIRECTORY/code_size_$REPORT_SUFFIX.txt
