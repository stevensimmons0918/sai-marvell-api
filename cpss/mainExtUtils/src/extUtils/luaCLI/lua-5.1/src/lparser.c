...
static void close_func (LexState *ls) {
  ...
  lua_assert(fs->bl == NULL);   ls->fs = fs->prev;
  ...
  do {
    ...
    switch(ls->t.token) {
      ...
      default: {  /* constructor_part -> listfield */
        if (ls->t.token == '[')  /* constructor_item -> recfield */
          recfield(ls, &cc);
        else
          listfield(ls, &cc);
        break;
...
static UnOpr getunopr (int op) {
  if (op == TK_NOT) return OPR_NOT;
  if (op == '-') return OPR_MINUS;
  if (op == '#') return OPR_LEN;
  return OPR_NOUNOPR;
}
