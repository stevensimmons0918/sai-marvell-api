...
int get_hw_drop_resolution(tm_handle hndl, uint8_t *p_resolution_mask)
{
	...
	*p_resolution_mask = 0;
	/* read register */
    TM_READ_REGISTER(TM.Drop.WREDDropProbMode, TM_Drop_WREDDropProbMode)
    if (rc) goto out;
...
int get_hw_b_node_params(tm_handle hndl, uint32_t index, struct b_node_hw_data_t *b_node_hw_data)
{
	...
	if (index < ctl->tm_total_b_nodes)
	{
		...
		{
		case 0:
			TM_REGISTER_GET(TM_Drop_BlvlDropProfPtr, ProfPtr0, b_node_hw_data->wred_profile_ref, (uint8_t));
			break;
		case 1:
			TM_REGISTER_GET(TM_Drop_BlvlDropProfPtr, ProfPtr1, b_node_hw_data->wred_profile_ref, (uint8_t));
			break;
		case 2:
			TM_REGISTER_GET(TM_Drop_BlvlDropProfPtr, ProfPtr2, b_node_hw_data->wred_profile_ref, (uint8_t));
			break;
		case 3:
			TM_REGISTER_GET(TM_Drop_BlvlDropProfPtr, ProfPtr3, b_node_hw_data->wred_profile_ref, (uint8_t));
			break;
		}
	}
	return rc;
