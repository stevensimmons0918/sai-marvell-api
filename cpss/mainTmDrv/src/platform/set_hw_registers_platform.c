...
int get_hw_shaping_status(tm_handle hndl, enum tm_level level , struct tm_level_periodic_params *periodic_params )
{
	...
	{
	case Q_LEVEL:
		TM_READ_REGISTER(TM.Sched.QueuePerConf, TM_Sched_QueuePerConf)
		if (rc)	goto out;
		/* assign register fields */
		...
		break;
	case A_LEVEL:
		TM_READ_REGISTER(TM.Sched.AlvlPerConf, TM_Sched_AlvlPerConf)
		if (rc)	goto out;
		/* assign register fields */
		...
		break;
	case B_LEVEL:
		TM_READ_REGISTER(TM.Sched.BlvlPerConf, TM_Sched_BlvlPerConf)
		if (rc)	goto out;
		/* assign register fields */
		...
		break;
	case C_LEVEL:
		TM_READ_REGISTER(TM.Sched.ClvlPerConf, TM_Sched_ClvlPerConf)
		if (rc)	goto out;
		/* assign register fields */
		...
		break;
	case P_LEVEL:
		TM_READ_REGISTER(TM.Sched.PortPerConf, TM_Sched_PortPerConf)
		if (rc)	goto out;
		/* assign register fields */
