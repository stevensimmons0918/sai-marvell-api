...
extern GT_U64  convert_tm_write_dummy;
extern GT_U64  convert_tm_read_dummy;
...
#define	TM_WRITE_REGISTER(address, register_name)	\
    ...
    tm_convert_to_u64(TM_REGISTER_VAR_ADDR(register_name),&convert_tm_write_dummy);\
    rc = tm_register_write(TM_ENV(ctl), REGISTER_ADDRESS_ARG(address),  &convert_tm_write_dummy);\
...
#define	TM_READ_REGISTER(address, register_name)	\
	...
	rc = tm_register_read(TM_ENV(ctl), REGISTER_ADDRESS_ARG(address),&convert_tm_read_dummy  );\
    tm_convert_from_u64(&convert_tm_read_dummy,TM_REGISTER_VAR_ADDR(register_name));\
...
#define	TM_WRITE_TABLE_REGISTER(address, index , register_name)	\
    ...
    tm_convert_to_u64(TM_REGISTER_VAR_ADDR(register_name),&convert_tm_write_dummy);\
    rc = tm_table_entry_write(TM_ENV(ctl), REGISTER_ADDRESS_ARG(address), index, &convert_tm_write_dummy);\
...
#define	TM_READ_TABLE_REGISTER(address, index , register_name)	\
	...
	rc = tm_table_entry_read(TM_ENV(ctl), REGISTER_ADDRESS_ARG(address), index,&convert_tm_read_dummy);\
	tm_convert_from_u64(&convert_tm_read_dummy,TM_REGISTER_VAR_ADDR(register_name));\
	if(tm_log_read_requests)\
	{\
		if (rc==0)\
