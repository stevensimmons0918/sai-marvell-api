...
/**
********************************************************************************
* @file wrapCpssPxPortAp.c
*
* @brief Wrapper functions for CPSS PX Port AP API
*
* @version   1
********************************************************************************
*/
...
/* counter for "get" functions of cpssPxPortApPortConfig table */
static GT_PHYSICAL_PORT_NUM   portConfigEntryCount;

/* counter for cpssPxPortApPortStatus table */
static GT_PHYSICAL_PORT_NUM   portStatusEntryCount;

/* counter for cpssPxPortApPortStats table */
...
)
{
    ...
    CONVERT_BACK_DEV_PORT_DATA_MAC(devNum, physPortNum);
    ...
    for(result = GT_OK; physPortNum < CPSS_MAX_PORTS_NUM_CNS; physPortNum++)
    {
        ...
        {
            result = cpssPxPortPhysicalPortDetailedMapGet(devNum, physPortNum,
                                                        /*OUT*/portMapShadowPtr);
            if (result != GT_OK)
            {
                return result;
            }
            ...
            if (PRV_CPSS_GE_PORT_GE_ONLY_E ==
                PRV_CPSS_PX_PORT_TYPE_OPTIONS_MAC(devNum, physPortNum))
            {
                continue;
            }
...
/**
* @internal wrCpssPxPortApEnableSet function
* @endinternal
*
...
* @retval GT_NOT_APPLICABLE_DEVICE - on not applicable device
*/
static CMD_STATUS wrCpssPxPortApEnableSet
(
...
)
{
    ...
    result = cpssPxPortApEnableSet(devNum, enable);
...
/**
* @internal wrCpssPxPortApEnableGet function
* @endinternal
*
...
* @retval GT_NOT_APPLICABLE_DEVICE - on not applicable device
*/
static CMD_STATUS wrCpssPxPortApEnableGet
(
    IN  GT_UINTPTR inArgs[CMD_MAX_ARGS],
    IN  GT_UINTPTR inFields[CMD_MAX_FIELDS],
    IN  GT_32 numFields,
...
)
{
    ...
    /* call cpss api function */
    result = cpssPxPortApEnableGet(devNum, &enable);
...
/**
* @internal wrCpssPxPortApPortConfigEntrySet function
* @endinternal
*
...
* @retval GT_NOT_APPLICABLE_DEVICE - on not applicable device
*/
static CMD_STATUS wrCpssPxPortApPortConfigEntrySet
(
    IN  GT_UINTPTR inArgs[CMD_MAX_ARGS],
    IN  GT_UINTPTR inFields[CMD_MAX_FIELDS],
    IN  GT_32 numFields,
...
)
{
    ...
    CPSS_PX_PORT_AP_PARAMS_STC  apParams, *apParamsPtr = NULL;
    ...
    for(i = laneNumOffset+2, j = 0; i < (laneNumOffset+2)+2*CPSS_PX_PORT_AP_IF_ARRAY_SIZE_CNS; i+=2, j++)
    {
...
config:
    ...
    if(GT_TRUE == apEnable)
    {
        ...
        else
        {
            ...
            if(result != GT_OK)
            {
                cpssOsPrintf("cpssPxPortInterfaceSpeedSupportGet(portNum=%d,RXAUI):rc=%d\n",
                                portNum, result);
                return result;
            }
            if(supported)
    ...
    /* call cpss api function */
    result = cpssPxPortApPortConfigSet(devNum, portNum, apEnable, apParamsPtr);
...
/**
* @internal prvWrCpssPxPortApPortConfigGet function
* @endinternal
*
...
* @retval GT_NOT_APPLICABLE_DEVICE - on not applicable device
*/
static CMD_STATUS prvWrCpssPxPortApPortConfigGet
(
    IN  GT_UINTPTR  inArgs[CMD_MAX_ARGS],
    IN  GT_UINTPTR  inFields[CMD_MAX_FIELDS],
    IN  GT_32       numFields,
...
)
{
    ...
    result = cpssPxPortApEnableGet(devNum, &apEnabledOnCore);
    if (result != GT_OK)
    {
        galtisOutput(outArgs, result, "%d", -1);
        return CMD_OK;
    ...
    if(apEnabledOnCore)
    {
        ...
        /* call cpss api function */
        result = cpssPxPortApPortConfigGet(devNum,portConfigEntryCount,&apEnable,
                                             &apParams);
        if (result != GT_OK)
        {
            galtisOutput(outArgs, result, "%d", -1);
    ...
    else
    {
        ...
        inFields[7] = CPSS_PORT_FEC_MODE_DISABLED_E;
        inFields[8] = CPSS_PORT_FEC_MODE_DISABLED_E;
    }
...
static CMD_STATUS wrCpssPxPortApPortConfigEntryGetFirst
(
    IN  GT_UINTPTR inArgs[CMD_MAX_ARGS],
    IN  GT_UINTPTR inFields[CMD_MAX_FIELDS],
    IN  GT_32 numFields,
...
)
{
    ...
    return prvWrCpssPxPortApPortConfigGet(inArgs,inFields,numFields,GT_FALSE,outArgs);
}

static CMD_STATUS wrCpssPxPortApPortConfigEntryGetNext
(
    IN  GT_UINTPTR inArgs[CMD_MAX_ARGS],
    IN  GT_UINTPTR inFields[CMD_MAX_FIELDS],
    IN  GT_32 numFields,
...
static CMD_STATUS wrCpssPxPortApPortConfigEntryEnhGetFirst
(
    IN  GT_UINTPTR inArgs[CMD_MAX_ARGS],
    IN  GT_UINTPTR inFields[CMD_MAX_FIELDS],
    IN  GT_32 numFields,
...
)
{
    ...
    return prvWrCpssPxPortApPortConfigGet(inArgs,inFields,numFields,GT_TRUE,outArgs);
}

static CMD_STATUS wrCpssPxPortApPortConfigEntryEnhGetNext
(
    IN  GT_UINTPTR inArgs[CMD_MAX_ARGS],
    IN  GT_UINTPTR inFields[CMD_MAX_FIELDS],
    IN  GT_32 numFields,
...
/**
* @internal prvWrCpssPxPortApPortStatusEntryGet function
* @endinternal
*
...
* @retval GT_NOT_APPLICABLE_DEVICE - on not applicable device
*/
static CMD_STATUS prvWrCpssPxPortApPortStatusEntryGet
(
    IN  GT_UINTPTR  inArgs[CMD_MAX_ARGS],
    IN  GT_UINTPTR  inFields[CMD_MAX_FIELDS],
    IN  GT_32       numFields,
...
)
{
    GT_STATUS                   result;
    GT_SW_DEV_NUM               devNum;
    CPSS_PX_PORT_AP_STATUS_STC  apStatus;
    GT_BOOL                     apEnabled;
    ...
    result = cpssPxPortApEnableGet(devNum,&apEnabled);
    if (result != GT_OK)
    {
        galtisOutput(outArgs, result, "%d", -1);
        return CMD_OK;
    ...
    if(apEnabled)
    {
        cpssOsMemSet(&apStatus, 0, sizeof(CPSS_PX_PORT_AP_STATUS_STC));
        /* call cpss api function */
        result = cpssPxPortApPortStatusGet(devNum, portStatusEntryCount,
                                             &apStatus);
        if (result != GT_OK)
        {
            galtisOutput(outArgs, result, "%d", -1);
        ...
        {
            result = cpssPxPortFecModeGet(devNum, portStatusEntryCount, &mode);
            if (result != GT_OK)
            {
                galtisOutput(outArgs, result, "%d", -1);
                return CMD_OK;
        ...
        {
            if (apStatus.fecType != CPSS_PORT_FEC_MODE_DISABLED_E)
            {
...
static CMD_STATUS  wrCpssPxPortApPortStatusEntryGetFirst
(
    IN  GT_UINTPTR inArgs[CMD_MAX_ARGS],
    IN  GT_UINTPTR inFields[CMD_MAX_FIELDS],
    IN  GT_32 numFields,
...
)
{
    ...
    return prvWrCpssPxPortApPortStatusEntryGet(inArgs, inFields, numFields,
                                                 GT_TRUE, outArgs);
}

static CMD_STATUS  wrCpssPxPortApPortStatusEntryGetNext
(
    IN  GT_UINTPTR inArgs[CMD_MAX_ARGS],
    IN  GT_UINTPTR inFields[CMD_MAX_FIELDS],
    IN  GT_32 numFields,
...
static CMD_STATUS  wrCpssPxPortApPortStatusOnPortGetFirst
(
    IN  GT_UINTPTR inArgs[CMD_MAX_ARGS],
    IN  GT_UINTPTR inFields[CMD_MAX_FIELDS],
    IN  GT_32 numFields,
...
/**
* @internal wrCpssPxPortApGetNext function
* @endinternal
*
...
* @retval GT_OK                    - on success
*/
static CMD_STATUS wrCpssPxPortApGetNext
(
    IN  GT_UINTPTR inArgs[CMD_MAX_ARGS],
    IN  GT_UINTPTR inFields[CMD_MAX_FIELDS],
    IN  GT_32 numFields,
...
/**
* @internal wrCpssPxPortApStatsReset function
* @endinternal
*
...
* @retval GT_NOT_APPLICABLE_DEVICE - on not applicable device
*/
static CMD_STATUS  wrCpssPxPortApStatsReset
(
    IN  GT_UINTPTR inArgs[CMD_MAX_ARGS],
    IN  GT_UINTPTR inFields[CMD_MAX_FIELDS],
    IN  GT_32 numFields,
...
)
{
    ...
    result = cpssPxPortApStatsReset(devNum, portNum);
...
/**
* @internal wrCpssPxPortApIntropSet function
* @endinternal
*
...
* @retval GT_NOT_APPLICABLE_DEVICE - on not applicable device
*/
static CMD_STATUS  wrCpssPxPortApIntropSet
(
    IN  GT_UINTPTR inArgs[CMD_MAX_ARGS],
    IN  GT_UINTPTR inFields[CMD_MAX_FIELDS],
    IN  GT_32 numFields,
...
)
{
    ...
    GT_SW_DEV_NUM               devNum;
    GT_PHYSICAL_PORT_NUM        portNum;
    CPSS_PX_PORT_AP_INTROP_STC  apIntropParams;
    ...
    result = cpssPxPortApIntropSet(devNum, portNum, &apIntropParams);
...
/**
* @internal wrCpssPxPortApIntropGet function
* @endinternal
*
...
* @retval GT_NOT_APPLICABLE_DEVICE - on not applicable device
*/
static CMD_STATUS  wrCpssPxPortApIntropGet
(
    IN  GT_UINTPTR inArgs[CMD_MAX_ARGS],
    IN  GT_UINTPTR inFields[CMD_MAX_FIELDS],
    IN  GT_32 numFields,
...
)
{
    ...
    result = cpssPxPortApIntropGet(devNum, portNum, &apIntropParams);
...
/**
* @internal prvWrCpssPxPortApPortsStatsGet function
* @endinternal
*
...
* @note   NOT APPLICABLE DEVICES:  Px1; Px1_Diamond; Px2; Px3; xCat; Lion; xCat2.
*
*
* @retval GT_OK                    - on success
* @retval GT_BAD_PARAM             - on wrong port number or device
...
* @retval GT_NOT_APPLICABLE_DEVICE - on not applicable device
*/
static CMD_STATUS prvWrCpssPxPortApPortsStatsGet
(
    IN  GT_UINTPTR  inArgs[CMD_MAX_ARGS],
    IN  GT_UINTPTR  inFields[CMD_MAX_FIELDS],
    IN  GT_32       numFields,
...
)
{
    GT_STATUS                   result;
    GT_SW_DEV_NUM               devNum;
    CPSS_PX_PORT_AP_STATS_STC   apStats;
    GT_BOOL                     apEnabled;
    ...
    cpssOsMemSet(&apStats, 0, sizeof(CPSS_PX_PORT_AP_STATS_STC));

    /* call cpss api function */
    result = cpssPxPortApStatsGet(devNum, portStatsEntryCount, &apStats, &intropAbilityMaxInterval);
    if (result != GT_OK)
    {
        galtisOutput(outArgs, result, "%d", -1);
        return CMD_OK;
...
static CMD_STATUS  wrCpssPxPortApPortsStatsGetFirst
(
    IN  GT_UINTPTR inArgs[CMD_MAX_ARGS],
    IN  GT_UINTPTR inFields[CMD_MAX_FIELDS],
    IN  GT_32 numFields,
...
)
{
    ...
    return prvWrCpssPxPortApPortsStatsGet(inArgs, inFields, numFields,
                                                 GT_TRUE, outArgs);
}

static CMD_STATUS  wrCpssPxPortApPortsStatsGetNext
(
    IN  GT_UINTPTR inArgs[CMD_MAX_ARGS],
    IN  GT_UINTPTR inFields[CMD_MAX_FIELDS],
    IN  GT_32 numFields,
...
static CMD_STATUS  wrCpssPxPortApPortStatsGetFirst
(
    IN  GT_UINTPTR inArgs[CMD_MAX_ARGS],
    IN  GT_UINTPTR inFields[CMD_MAX_FIELDS],
    IN  GT_32 numFields,
...
/**
* @internal wrCpssPxPortApPortStatsGetNext function
* @endinternal
*
* @brief   This function is called for ending table printing.
*
...
* @retval GT_OK                    - on success
*/
static CMD_STATUS wrCpssPxPortApPortStatsGetNext
...
/**
* @internal wrCpssPxPortApPortConfigEntryFecExtSet function
* @endinternal
*
* @brief   Enable/disable AP process on port.
*
...
* @retval GT_NOT_APPLICABLE_DEVICE - on not applicable device
*/
static CMD_STATUS wrCpssPxPortApPortConfigEntryFecExtSet
(
    IN  GT_UINTPTR inArgs[CMD_MAX_ARGS],
    IN  GT_UINTPTR inFields[CMD_MAX_FIELDS],
    IN  GT_32 numFields,
...
)
{
    ...
    for(i = 9, j = 0; i < 9+4*CPSS_PX_PORT_AP_IF_ARRAY_SIZE_CNS; i+=4, j++)
    {
...
config:
    ...
    if(GT_TRUE == apEnable)
    {
        ...
        if(result != GT_OK)
        {
            cpssOsPrintf("cpssPxPortInterfaceSpeedSupportGet(portNum=%d,XGMII):rc=%d\n",
                            portNum, result);
            return result;
        }
...
/**
* @internal prvWrCpssPxPortApPortConfigFecExtGet function
* @endinternal
*
* @brief   Get AP configuration of port.
*
...
* @retval GT_NOT_APPLICABLE_DEVICE - on not applicable device
*/
static CMD_STATUS prvWrCpssPxPortApPortConfigFecExtGet
(
    IN  GT_UINTPTR  inArgs[CMD_MAX_ARGS],
    IN  GT_UINTPTR  inFields[CMD_MAX_FIELDS],
    IN  GT_32       numFields,
    ...
    if(apEnabledOnCore)
    {
        ...
        for(i = 9, j = 0; i < 9+4*CPSS_PX_PORT_AP_IF_ARRAY_SIZE_CNS; i+=4, j++)
        {
            ...
            inFields[i+2] = (0 == apParams.modesAdvertiseArr[j].ifMode) ?
                                CPSS_PORT_FEC_MODE_DISABLED_E : apParams.fecAbilityArr[j];
            inFields[i+3] = (0 == apParams.modesAdvertiseArr[j].ifMode) ?
                                CPSS_PORT_FEC_MODE_DISABLED_E : apParams.fecRequestedArr[j];
        }
    }
    else
    {
        ...
        for(i = 9; i < 9+4*CPSS_PX_PORT_AP_IF_ARRAY_SIZE_CNS; i+=4)
        {
            inFields[i] = 0;
            inFields[i+1] = 0;
            inFields[i+2] = CPSS_PORT_FEC_MODE_DISABLED_E;
            inFields[i+3] = CPSS_PORT_FEC_MODE_DISABLED_E;
        }
    }
    inFields[0] = portConfigEntryCount;
    inFields[2] = GT_FALSE; /* useCpssDefault */
...
static CMD_STATUS wrCpssPxPortApPortConfigEntryFecExtGetFirst
(
    IN  GT_UINTPTR inArgs[CMD_MAX_ARGS],
    IN  GT_UINTPTR inFields[CMD_MAX_FIELDS],
    IN  GT_32 numFields,
...
)
{
    ...
    return prvWrCpssPxPortApPortConfigFecExtGet(inArgs,inFields,numFields,outArgs);
}

static CMD_STATUS wrCpssPxPortApPortConfigEntryFecExtGetNext
(
    IN  GT_UINTPTR inArgs[CMD_MAX_ARGS],
    IN  GT_UINTPTR inFields[CMD_MAX_FIELDS],
    IN  GT_32 numFields,
...
static CPSS_PX_PORT_AP_PARAMS_STC   apPortsArray[CPSS_MAX_PORTS_NUM_CNS];
...
GT_STATUS prvApPxConfigApPorts
(
    GT_U8                       devNum,
    GT_BOOL                     apEnable,
    GT_U32                      numOfPortsToConfig,
    CPSS_PORTS_BMP_STC          *apPrtBitmapPtr,
    CPSS_PX_PORT_AP_PARAMS_STC  *apPortsArrPtr,
...
)
{
    ...
    for(portNum = 0, numOfPortsConfig = 0; portNum < CPSS_MAX_PORTS_NUM_CNS && numOfPortsConfig < numOfPortsToConfig; portNum++)
    {
        ...
        if(GT_TRUE == apEnable)
        {
            ...
            if(result != GT_OK)
            {
                cpssOsPrintSync("cpssPxPortInterfaceSpeedSupportGet(portNum=%d,XGMII):rc=%d\n",
                                portNum, result);
                return result;
            }
            ...
            else
            {
                ...
                if(result != GT_OK)
                {
                    cpssOsPrintSync("cpssPxPortInterfaceSpeedSupportGet(portNum=%d,RXAUI):rc=%d\n",
                                    portNum, result);
                    return result;
                }
                if(supported)
        ...
        {
            result = cpssPxPortApPortConfigSet(devNum, portNum, apEnable, NULL);
        }
        else
        {
            result = cpssPxPortApPortConfigSet(devNum, portNum, apEnable, &apPortsArrPtr[numOfPortsConfig]);
        }

        if(result != GT_OK)
        {
            cpssOsPrintSync("cpssPxPortApPortConfigSet: error unable to config port %d, error = %d\n",
                            portNum, result);
            return result;
        }
        else
        {
            cpssOsPrintSync("cpssPxPortApPortConfigSet: config port %d\n",portNum);
        }
        numOfPortsConfig++;
    }
...
* @note   NOT APPLICABLE DEVICES:  Px1; Px1_Diamond; Px2; Px3; xCat; xCat3; Lion; Lion2; xCat2.
*
*
* @retval GT_OK                    - on success
...
)
{
    ...
    else if(opCode == 0xFFFFFFFF)/*end registration*/
    {
        ...
        result = prvApPxConfigApPorts(devNum, apEnable, numOfApPortsToConfig, &apPortBitmap, apPortsArray, useCpssDefaultsArray);
        if(result != GT_OK)
        {
    ...
    if(useCpssDefaultsArray[numOfApPortsToConfig] == GT_FALSE)
    {
        ...
        apPortsArray[numOfApPortsToConfig].fcPause = (CPSS_PX_PORT_AP_FLOW_CONTROL_ENT)inFields[3];
        ...
        for(i = 8, j = 0; j < CPSS_PX_PORT_AP_IF_ARRAY_SIZE_CNS; i+=2, j++)
        {
...
static CMD_STATUS wrCpssPxPortApPortsSetFirst
(
    IN  GT_UINTPTR inArgs[CMD_MAX_ARGS],
    IN  GT_UINTPTR inFields[CMD_MAX_FIELDS],
    IN  GT_32 numFields,
...
static CMD_STATUS wrCpssPxPortApPortsSetNext
(
    IN  GT_UINTPTR inArgs[CMD_MAX_ARGS],
    IN  GT_UINTPTR inFields[CMD_MAX_FIELDS],
    IN  GT_32 numFields,
...
static CMD_STATUS wrCpssPxPortApPortsEndSet
(
    IN  GT_UINTPTR inArgs[CMD_MAX_ARGS],
    IN  GT_UINTPTR inFields[CMD_MAX_FIELDS],
    IN  GT_32 numFields,
...
/**
* @internal wrCpssPxPortApDebugInfoEnable function
* @endinternal
*
* @brief   Enable/disable application thread for AP ports real time logging.
*
* @note   APPLICABLE DEVICES:      BC2; BOBK.
* @note   NOT APPLICABLE DEVICES:  Px1; Px1_Diamond; Px2; Px3; xCat; Lion; xCat2; Lion2.
*
*
* @retval GT_OK                    - on success
* @retval GT_BAD_PARAM             - on wrong device
...
* @retval GT_NOT_APPLICABLE_DEVICE - on not applicable device
*/
static CMD_STATUS wrCpssPxPortApDebugInfoEnable
...
#if 0 /* appDemoPxPortApLinkUpDebug */

/**
* @internal wrCpssPxPortApLinkUpDebug function
* @endinternal
*
...
static CMD_STATUS wrCpssPxPortApLinkUpDebug
(
    IN  GT_UINTPTR inArgs[CMD_MAX_ARGS],
    IN  GT_UINTPTR inFields[CMD_MAX_FIELDS],
    IN  GT_32 numFields,
...
)
{
    ...
    CONVERT_SW_DEV_PHYSICAL_PORT_MAC(devNum, portNum0);
    /* Override Device and Port */
    CONVERT_SW_DEV_PHYSICAL_PORT_MAC(devNum, portNum1);

    rc = appDemoPxPortApLinkUpDebug(devNum,portNum0,portNum1,speed,
                ifMode,noneceDisableValue,timesLoop, linkCheckDelay, enDelay, disDelay);
    if (rc != GT_OK)
    {
        galtisOutput(outArgs, rc, "appDemoPxPortApLinkUpDebug");
        return CMD_OK;
    }
...
#endif /* appDemoPxPortApLinkUpDebug */

/**** database initialization **************************************/

static CMD_COMMAND dbCommands[] =
{
    {"cpssPxPortApEnableSet",
        &wrCpssPxPortApEnableSet,
        2, 0},

    {"cpssPxPortApEnableGet",
        &wrCpssPxPortApEnableGet,
        1, 0},

    {"cpssPxPortApPortConfigEntrySet",
        &wrCpssPxPortApPortConfigEntrySet,
        1, 31},
    {"cpssPxPortApPortConfigEntryGetFirst",
        &wrCpssPxPortApPortConfigEntryGetFirst,
        1, 0},
    {"cpssPxPortApPortConfigEntryGetNext",
        &wrCpssPxPortApPortConfigEntryGetNext,
        1, 0},

    {"cpssPxPortApPortConfigEntryEnhFecSet",
        &wrCpssPxPortApPortConfigEntrySet,
        1, 31},
    {"cpssPxPortApPortConfigEntryEnhFecGetFirst",
        &wrCpssPxPortApPortConfigEntryEnhGetFirst,
        1, 0},
    {"cpssPxPortApPortConfigEntryEnhFecGetNext",
        &wrCpssPxPortApPortConfigEntryEnhGetNext,
        1, 0},

    {"cpssPxPortApPortStatusEntryEnhFecGetFirst",
        &wrCpssPxPortApPortStatusEntryGetFirst,
        1, 0},
    {"cpssPxPortApPortStatusEntryEnhFecGetNext",
        &wrCpssPxPortApPortStatusEntryGetNext,
        1, 0},

    {"cpssPxPortApPortStatusOnPortGetFirst",
        &wrCpssPxPortApPortStatusOnPortGetFirst,
        2, 0},
    {"cpssPxPortApPortStatusOnPortGetNext",
        &wrCpssPxPortApGetNext,
        2, 0},

    {"cpssPxPortApStatsGetFirst",
        &wrCpssPxPortApPortsStatsGetFirst,
        1, 0},
    {"cpssPxPortApStatsGetNext",
        &wrCpssPxPortApPortsStatsGetNext,
        1, 0},
    {"cpssPxPortApStatsOnPortGetFirst",
        &wrCpssPxPortApPortStatsGetFirst,
        2, 0},
    {"cpssPxPortApStatsOnPortGetNext",
        &wrCpssPxPortApPortStatsGetNext,
        2, 0},
    {"cpssPxPortApStatsReset",
        &wrCpssPxPortApStatsReset,
        2, 0},
    {"cpssPxPortApIntropSet",
        &wrCpssPxPortApIntropSet,
        11, 0},
    {"cpssPxPortApIntropGet",
        &wrCpssPxPortApIntropGet,
        2, 0},

    {"cpssPxPortApPortConfigEntryFecExtSet",
        &wrCpssPxPortApPortConfigEntryFecExtSet,
        1, 49},
    {"cpssPxPortApPortConfigEntryFecExtGetFirst",
        &wrCpssPxPortApPortConfigEntryFecExtGetFirst,
        1, 0},
    {"cpssPxPortApPortConfigEntryFecExtGetNext",
        &wrCpssPxPortApPortConfigEntryFecExtGetNext,
        1, 0},

    {"cpssPxPortApPortConfigEntriesFecExtSetFirst",
        &wrCpssPxPortApPortsSetFirst,
        2, 48},
    {"cpssPxPortApPortConfigEntriesFecExtSetNext",
        &wrCpssPxPortApPortsSetNext,
        2, 48},
    {"cpssPxPortApPortConfigEntriesFecExtEndSet",
        &wrCpssPxPortApPortsEndSet,
        2, 0},

#if 0
    ,{"cpssPxPortApPortConfigEntriesSetFirst",
        &wrCpssPxPortApPortsSetFirst,
        2, 27},
    {"cpssPxPortApPortConfigEntriesSetNext",
        &wrCpssPxPortApPortsSetNext,
        2, 27},
    {"cpssPxPortApPortConfigEntriesEndSet",
        &wrCpssPxPortApPortsEndSet,
        2, 0},
    {"cpssPxPortApDebugInfoEnable",
        &wrCpssPxPortApDebugInfoEnable,
        3, 0},
    {"cpssPxPortApLinkUpDebugSet",
        &wrCpssPxPortApLinkUpDebug,
        10, 0}
#endif
};
...
/**
* @internal cmdLibInitCpssPxPortAp function
* @endinternal
*
...
*
*/
GT_STATUS cmdLibInitCpssPxPortAp
(
    GT_VOID
)
{
