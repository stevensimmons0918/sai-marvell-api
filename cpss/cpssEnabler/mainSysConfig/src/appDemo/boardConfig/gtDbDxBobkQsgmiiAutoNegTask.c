/**
********************************************************************************
* @file gtDbDxBobkQsgmiiAutoNegTask.c
*
* @brief AutoNegotiation work-around task implementation for BobK (Cetus/Caelum)
*
* @version   1
********************************************************************************
*/
...
*
* DESCRIPTION:
*       AutoNegotiation work-around task implementation for BobK (Cetus/Caelum)
*
 ...
 *--------------------------------------------------*/
    ...
    (
        IN GT_U8 devNum,
        INGT_PHYSICAL_PORT_NUM portNum,
        IN CPSS_EVENT_MASK_SET_ENT state
    )
    {
        GT_STATUS rc;
        ...
        rc = cpssEventDeviceMaskWithEvExtDataSet(devNum, CPSS_PP_PORT_SYNC_STATUS_CHANGED_E, portNum, state);
        if (rc != GT_OK)
        {
            return rc;
        }
        rc = cpssEventDeviceMaskWithEvExtDataSet(devNum, CPSS_PP_PORT_AN_COMPLETED_E, portNum, state);
        if (rc != GT_OK)
        {
            return rc;
        }
...
)
{
    ...
    rc = AN_WA_PortAutoNegInterruptsStateByMacSet(devNum, portNum, state);
    if (rc != GT_OK)
    {
        return rc;
    }
...
)
{
    ...
    if (g_anWa_TaskData.taskCreated == GT_TRUE)
    {
        ...
        {
            /*-------------------------------------------------------------------*
             * if codewords are same, ignore current interrupt and allow next
             * otherwise proceeds with the interrupt
             *-------------------------------------------------------------------*/
...
)
{
    ...
    PRV_CPSS_DXCH_DEV_CHECK_MAC(devNum);
    PRV_CPSS_NOT_APPLICABLE_DEV_CHECK_MAC(devNum, CPSS_LION2_E | CPSS_XCAT3_E | CPSS_BOBCAT2_E );
...
GT_STATUS AN_TASK_UserOptionPrint
(
    GT_U8 devNum
)
{
    ...
    cpssOsPrintf("\n+--------------------------------------------+");
    cpssOsPrintf("\n|   port AN WA data base                     |");
    cpssOsPrintf("\n+------+-----+-------+----------+------------+");
    cpssOsPrintf("\n|      |     | Speed |  Duplex  | Flow Ctrl  |");
    cpssOsPrintf("\n| port | mac +-------+----------+----+---+---+");
    cpssOsPrintf("\n|      |     | En    | En |State| En |Sym|Asm|");
    cpssOsPrintf("\n+------+-----+-------+----------+------------+");
    isAnyPort = GT_FALSE;
    for (portNum = 0; portNum < CPSS_MAX_PORTS_NUM_CNS; portNum++)
    {
        ...
        if (paramsPtr->antoNegEnable == GT_TRUE)
        {
            rc = cpssDxChPortPhysicalPortDetailedMapGet(devNum,portNum,/*OUT*/&portMapShadow);
            if (rc != GT_OK)
            {
            ...
            cpssOsPrintf("\n");
            cpssOsPrintf("| %4d ",portNum);
            if (portMapShadow.valid == GT_TRUE)
            {
                cpssOsPrintf("| %3d ",portMapShadow.portMap.macNum);
            }
            else
            {
                cpssOsPrintf("| %3s ","---");
            }
            cpssOsPrintf("| %5d ",paramsPtr->autoNegSpeedEn);
            cpssOsPrintf("| %2d ",paramsPtr->autoNegDuplexEn);
            cpssOsPrintf("| %2s ",paramsPtr->autoNegDuplexMode == 1 ? "FD":"HD");
            cpssOsPrintf("| %2d ",paramsPtr->autoNegFlowCtrlEn);
            cpssOsPrintf("| %1d ",paramsPtr->autoNegFlowCtrlPauseAdv);
            cpssOsPrintf("| %1d ",paramsPtr->autoNegFlowCtrlPauseAsmAdv);
            cpssOsPrintf("|");
            isAnyPort = GT_TRUE;
            if (portMapShadow.valid == GT_FALSE)
            {
                cpssOsPrintf("* something wrong");
            }
        }
    }
    if (isAnyPort == GT_TRUE)
    {
        cpssOsPrintf("\n+------+-------+----------+------------+");
    }
    else
    {
        cpssOsPrintf("\nNo ports enabled for AN");
    }
    cpssOsPrintf("\n\n");
    return GT_OK;
}
...
(
    ...
    GT_STATUS AN_WA_Test_InitSystem(GT_BOOL debugPrint)
    {
        ...
        rc = cpssDxChPortPhysicalPortMapReverseMappingGet(devNum, CPSS_DXCH_PORT_MAPPING_TYPE_ETHERNET_MAC_E, 56,/*OUT*/&portNum);
        if(GT_OK != rc)
        {
            return rc;
        }
