                                ...
                                MTD_U8 data[],MTD_U32 size, MTD_U16 *errCode)
{
    ...
    MTD_U32 numTransfers, lastTransferSize, transferIndex;
    MTD_U16 maxBuffSize = 0;
    ...
    *errCode = 0;
    ...
    TRY_IO(mtdHwXmdioRead(contextPtr, port, MAX_BUFF_SIZE_OUT_REG, &maxBuffSize));
    ...
    {        
        ...
        TRY_IO(mtdHwXmdioWrite(contextPtr, port,ACTUAL_BUFF_SIZE_IN_REG, maxBuffSize/2));
                                 ...
                                 MTD_U16 *errCode)
{
    ...
    TRY_IO(mtdHwXmdioRead(contextPtr, ports[0], MAX_BUFF_SIZE_OUT_REG, &maxBuffSize));
    ...
    for(transferIndex=0; transferIndex < numTransfers; transferIndex++)
    {
        ...
        /* Set the size of the buffer we're going to send*/
        TRY_IO(mtdHwXmdioWrite(contextPtr, ports[0],ACTUAL_BUFF_SIZE_IN_REG, maxBuffSize/2));
        /* Tell the slave we've written the start address and size
        / and now we're going to start writing data*/
...
)
{
    ...
    for(i = 0; i < numPorts; i++)
    {
        if(erroredPorts[i] == 0)
        {
            ...
            {
                printf(".");
                ATTEMPT(mtdWait(contextPtr, 500)); /* don't poll too often, this can take awhile */
                TRY_IO(mtdHwXmdioRead(contextPtr, ports[i],COMMAND_REG,&tmp));
            }
            printf("\n");
            if (tmp == MTD_SLAVE_ERR_CMD)
            {
                printf("Slave encountered error while erasing flash on port %d. \n",ports[i]);
                erroredPorts[i] = MTD_ERR_ERASING_FLASH;        
            }
            else
            {
                if (tmp == MTD_SLAVE_OK)
                {
                    printf("Flash program areas have been erased for port %d.\n",ports[i]);
                }
                else
                {
                    /* unexpected value read back from download code*/
                    printf("Unexpected response from phy on port %d is 0x%04X \n",ports[i],(unsigned)tmp);
                    erroredPorts[i] = MTD_ERR_VALUE_READ_BACK;
                }
            }
        }
...
MTD_STATUS mtdParallelEraseFlashImage
(
    IN CTX_PTR_TYPE contextPtr,
    IN MTD_U16 ports[],
    IN MTD_U32 appSize, 
