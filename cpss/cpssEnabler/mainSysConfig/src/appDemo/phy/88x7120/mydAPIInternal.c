...
)
{
    ...
    {
        if ((opMode == MYD_R50U || opMode == MYD_R400U)) /* work-in-progress */
        {
            if (autoNeg == MYD_AN_TRAIN_ON)
            {
...
/*******************************************************************
 MYD_STATUS mydOEMEnhancement
 Enhancement for R50U and R400U only
*******************************************************************/
MYD_STATUS mydOEMEnhancement
(
    IN MYD_DEV_PTR pDev,
...
)
{
    ...
    /* for R50U and R400U only */
    if (!(opMode == MYD_R50U || opMode == MYD_R400U))
    {
        return MYD_OK;
    }

    ATTEMPT(mydHwXmdioWrite(pDev, mdioPort, host_or_line, 0xF220+(0x80*laneOffset), 0x6C2C));
    ATTEMPT(mydHwXmdioWrite(pDev, mdioPort, host_or_line, 0xF230+(0x80*laneOffset), 0x0907));

    /* set HF min limit */
    minHF = (host_or_line == MYD_HOST_SIDE)? 0x0:0x3;
    ATTEMPT(mydHwXmdioWrite(pDev, mdioPort, host_or_line, 0xF231+(0x80*laneOffset), minHF));

    ATTEMPT(mydHwSetPhyRegField(pDev, mdioPort, host_or_line, 0xF250+(0x80*laneOffset), 0, 8, 0x11));
    ATTEMPT(mydHwSetPhyRegField(pDev, mdioPort, host_or_line, (0xF25E)+(0x80*laneOffset), 0, 2, 0x3));
...
)
{
    ...
    if (opMode == MYD_R50U || opMode == MYD_R400U)
    {
        ATTEMPT(mydOEMEnhancement(pDev, mdioPort, host_or_line, laneOffset, opMode)); 
    }
#endif
...
)
{
    ...
    switch(pDev->deviceId)
    {
        ...
        default:
            MYD_DBG_ERROR("mydCheckSupportedPinMode: Unknown or Unsupport device model: 0x%x\n", pDev->deviceId);
            return MYD_FAIL;
    }
...
)
{
    ...
    if (pDev->deviceId == MYD_DEV_X7120)
    {
        switch(pinId)
        {
            ...
            case MYD_PIN_GPIO2:
                ...
                {
                    /* Not possible, but incase we reach here. */
            ...
            case MYD_PIN_CLK_OUT_DIFF_P:
                ...
                /* Only supports Port3, no selection needed, already checked in the mydRClkConfigInputCheck() */
                if (portSelect != pDev->mdioPort+3)
                {
    ...
    /* Not possible but incase we are here, to avoid array index access out of range */
