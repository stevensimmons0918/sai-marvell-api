...
static PRV_MMAP_AREA_INFO_STC prvMmapAreaInfo[] = {
    ...
    { LINUX_VMA_PP_CONF_BASE,              0x1400000/*20M*/, "MVPPCONF" },
    { LINUX_VMA_PP_DFX_BASE,               0x2000000/*32M*/, "MVPPDFX" },
...
/**
* @internal check_overlap function
* @endinternal
*
* @brief   Check if /proc/self/maps region overlaps with CPSS shared lib mappings
*         Display an error message if overlapped
* @param[in] str                      - a single line from /proc/self/maps
* @param[in] start                    -  of a memory region
* @param[in] end                      -  of a memory region
*                                       None
*/
...
/**
* @internal shmDoCheckOverlap function
* @endinternal
*
* @brief   Read /proc/self/maps for a possible overlaps with CPSS shared lib mappings
*
...
)
{
    ...
    while(fgets(buf, sizeof(buf), map) != NULL)
    {
        ...
        else
        {
            ...
            /* check_overlap(buf, start, done); */
            /* Start address matches the key? */
            if(NULL!= strstr(buf,"libcpss.so"))
            {
                sectPtr = cpss_sectPtr;
                sectPtr->startPtr = (GT_VOID *)((GT_UINTPTR)start);
                sectPtr->donePtr  = (GT_VOID *)((GT_UINTPTR)done);
            } else if(NULL!= strstr(buf,"libhelper.so"))
            {
                sectPtr = mainos_sectPtr;
                sectPtr->startPtr = (GT_VOID *)((GT_UINTPTR)start);
                sectPtr->donePtr  = (GT_VOID *)((GT_UINTPTR)done);
...
#define DO_RM(name, suff,ind) \
    ...
    DO_RM(SHARED_MEMORY_MASTER_PID_FILE,0,8);
#undef DO_RM
