...
local function negotiation_func(params)
    ...
    if true == command_data["status"] then
        ...
        for iterator, devNum, portNum in command_data:getInterfacePortIterator() do
            ...
            result, values = cpssPerPortParamSet("cpssPxPortSpeedAutoNegEnableSet",
                                                 devNum, portNum,
			...
			if (true == command_data["local_status"]) and
			   (true == port_speed_changing) then
				result, values = cpssPerPortParamSet("cpssPxPortSpeedSet",
													 devNum, portNum,
													 port_speed, "speed",
													 "CPSS_PORT_SPEED_ENT")
				if  0x10 == result then
            ...
            result, values = cpssPerPortParamSet("cpssPxPortDuplexAutoNegEnableSet",
                                                 devNum, portNum,
            ...
            if (true == command_data["local_status"]) and
               (true == port_duplex_changing) then
                result, values = cpssPerPortParamSet("cpssPxPortDuplexModeSet",
                                                     devNum, portNum,
                                                     port_duplex, "dMode",
