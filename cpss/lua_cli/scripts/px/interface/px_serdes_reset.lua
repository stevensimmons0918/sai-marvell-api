                ...
                if(ret==GT_OK) then
                    ...
                    if(ret==GT_OK) then
                        ...
                        if(numOfSerdesLanesPtr~=0) then
                            ret = myGenWrapper("cpssPxPortSerdesResetStateSet",{
                                {"IN","GT_SW_DEV_NUM","devNum",devNum},
                                {"IN","GT_PHYSICAL_PORT_NUM","portNum",portNum},
                                {"IN","GT_BOOL","state",state}})

                            if(ret~=GT_OK) then
                                print(tostring(devNum).."/"..tostring(portNum).." Error cpssPxPortSerdesResetStateSet ret "..to_string(ret).." = "..returnCodes[ret])
                            --else
                            --    print(tostring(devNum).."/"..tostring(portNum).." cpssPxPortSerdesResetStateSet Reset = "..to_string(state))
                            end
                        end
                      end
                end
