...
local function trafficGeneratorConfigure(params)
    ...
    configPtr=
    {
    macDa={[0]=tonumber(string.sub(macDaStr,1,2)),
               tonumber(string.sub(macDaStr,4,5)),
               tonumber(string.sub(macDaStr,7,8)),
               tonumber(string.sub(macDaStr,10,11)),
               tonumber(string.sub(macDaStr,13,14)),
               tonumber(string.sub(macDaStr,16,17))
           },
    macSa={[0]=tonumber(string.sub(macSaS,1,2)),
               tonumber(string.sub(macSaS,4,5)),
               tonumber(string.sub(macSaS,7,8)),
               tonumber(string.sub(macSaS,10,11)),
               tonumber(string.sub(macSaS,13,14)),
               tonumber(string.sub(macSaS,16,17))
           },
    ...
    if nil~=params["payload"] then
        if "payload-random"==params["payload"] then
            ...
            --convert number to hexa
            local s = params["payload-cyclic"]
            local prefix = ""
            for i=1,128-#s do --padding the number to 128 digits
                prefix = prefix.."0"
            end
            s=prefix..s
            ...
            for ii=1,64 do
                params["payload-cyclic"][64-ii] = tonumber("0x"..string.sub(s,ii*2-1, ii*2))
            end
        end
    else  --default
        configPtr["payloadType"]="CPSS_DIAG_PG_PACKET_PAYLOAD_CYCLIC_E"
...
local function trafficGeneratorConnect(params)
    ...
    for devNum, dev_ports in pairs(all_ports) do
        for k, port in pairs(dev_ports) do
            ret,val = myGenWrapper("cpssPxDiagPacketGeneratorConnectGet",{  --get configure:
                {"IN","GT_SW_DEV_NUM","devNum",devNum},
                {"IN","GT_PHYSICAL_PORT_NUM","portNum",port},
            ...
            if 0~=ret then
                ...
                if configPtr.packetLength == 0 then
                    configPtr.packetLength = 20
                end
                if configPtr.packetCount == 0 then
                    configPtr.packetCount = 1
                end
                ...
                ret = myGenWrapper("cpssPxDiagPacketGeneratorConnectSet",{  --set connection:
                    {"IN","GT_SW_DEV_NUM","devNum",devNum},
                    {"IN","GT_PHYSICAL_PORT_NUM","portNum",port},
                    {"IN","GT_BOOL","connectPtr",connect},
                    {"IN","CPSS_PX_DIAG_PG_CONFIGURATIONS_STC","configPtr", configPtr}
        ...
        for k, port in pairs(dev_ports) do
            ...
            if 0~=ret then
                ...
                if false==val["connectPtr"]  then
                    ...
                    ret = myGenWrapper("cpssPxDiagPacketGeneratorTransmitEnable",{  --set configure:
                    {"IN","GT_SW_DEV_NUM","devNum",devNum},
                    {"IN","GT_PHYSICAL_PORT_NUM","portNum",port},
...
CLI_addCommand("interface", "traffic-generator configure", {
  ...
  params={
    {type="named",
      ...
      { format="increment", name="increment", help="Increment destination MAC for every packet" },
