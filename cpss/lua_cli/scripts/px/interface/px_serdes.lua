...
local function serdes_calibration_func(params)
    ...
    for iterator, devNum, portNum in command_data:getPortIterator() do
        ...
        result, values = myGenWrapper("cpssPxSamplePortManagerOpticalCalibrationSet",{
        ...
        elseif result~=GT_OK then
            print("Error at command: cpssPxSamplePortManagerOpticalCalibrationSet :%s", result)
        end
    end
...
local function serdes_etl_func(params)
    ...
    for iterator, devNum, portNum in command_data:getPortIterator() do
        ...
        result, values = myGenWrapper("cpssPxSamplePortManagerEnhanceTuneOverrideSet",{
                  {"IN","GT_U8","devNum",devNum},
                  {"IN","GT_PHYSICAL_PORT_NUM","portNum",portNum},
                  {"IN","GT_U32", "minLf", minLF},
                  {"IN","GT_U32", "maxLf", maxLF}})
        ...
        elseif result == GT_BAD_STATE then
            print("Error : Port is not in reset state \n")
        elseif result~=GT_OK then
            print("Error at command: cpssPxSamplePortManagerEnhanceTuneOverrideSet :%s", result)
        end
    end
    command_data:analyzeCommandExecution()
    command_data:printCommandExecutionResults()
    ...
    for iterator, devNum, portNum in command_data:getPortIterator() do
        ...
        if params.flagNo == true then
            ...
            elseif params.auto_tune_rx == "CPSS_PORT_SERDES_AUTO_TUNE_MODE_ADAPTIVE_RX_TRAINING_START_E" then
                ...
                result, values = myGenWrapper("cpssPxSamplePortManagerAdaptiveRxTrainSuppSet",{
                                             {"IN", "GT_U8","devNum",devNum},
                                             {"IN", "GT_PHYSICAL_PORT_NUM","portNum",portNum},
                                             {"IN", "GT_BOOL", "adaptRxTrainSupp", adaptRxTrainSupp}})
            else
                edgeDetectSupported = GT_FALSE
                result, values = myGenWrapper("cpssPxSamplePortManagerEdgeDetectSuppSet",{
                                             {"IN", "GT_U8","devNum",devNum},
                                             {"IN", "GT_PHYSICAL_PORT_NUM","portNum",portNum},
        ...
        result, values = myGenWrapper("cpssPxSamplePortManagerTrainModeSet",{
                  {"IN","GT_U8","devNum",devNum},
                  {"IN","GT_PHYSICAL_PORT_NUM","portNum",portNum},
        ...
        elseif result == GT_BAD_STATE then
            print("Error : Port is not in reset state \n")
        elseif result~=GT_OK then
            print("Error at command: cpssPxSamplePortManagerTrainModeSet :%s", result)
        end
    end
    command_data:analyzeCommandExecution()
    command_data:printCommandExecutionResults()
