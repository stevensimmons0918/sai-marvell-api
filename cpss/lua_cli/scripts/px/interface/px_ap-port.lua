...
local function ap_func(params)
  ...
  if fecSupported ~= "CPSS_PORT_FEC_MODE_DISABLED_E" then
      apParamsPtr.fecSupported = true
  else
      apParamsPtr.fecSupported = false
  end

  if fecRequested ~= "CPSS_PORT_FEC_MODE_DISABLED_E" then
  ...
  if true == command_data["status"] then
      ...
      if(nil~=devNum) then
        ret = myGenWrapper("cpssPxPortApPortConfigSet",{
          ...
          {"IN","CPSS_PX_PORT_AP_PARAMS_STC","apParamsPtr",apParamsPtr}})
        if(ret~=GT_OK) then
    ...
    if params.flagNo then
        for iterator, devNum, portNum in command_data:getPortIterator() do
            ...
            if params.advertisement~=nil then
                ...
                if(result~=GT_OK) then
                    print("Error at command: cpssPxPortManagerPortParamsGet :%d", result)
                end
                ...
                if params.nonce_match~=nil or params.flow_control~=nil or params.lane_num~=nil then
                    result, values = myGenWrapper("cpssPxSamplePortManagerApAttributeParamsSet",
                              {{ "IN", "GT_U8"  , "devNum", devNum},
                               { "IN", "GT_PHYSICAL_PORT_NUM", "portNum", portNum},
                   ...
                   elseif result~=GT_OK then
                       print("Error at command: cpssPxSamplePortManagerApAttributeParamsSet:%s", result)
                   end
                else
                    event.portEvent =  "CPSS_PORT_MANAGER_EVENT_DELETE_E"
                    result, values = myGenWrapper("cpssPxPortManagerEventSet",
        ...
        for iterator, devNum, portNum in command_data:getPortIterator() do
            ...
            if params.advertisement~=nil then
                ...
                result, values = myGenWrapper("cpssPxSamplePortManagerApMandatoryParamsSet",
                          {{ "IN", "GT_U8"  , "devNum", devNum},
                           { "IN", "GT_PHYSICAL_PORT_NUM", "portNum", portNum},
                ...
                elseif result == GT_BAD_STATE then
                    print("Error : Port is not in reset state \n")
                elseif result~=GT_OK then
                    print("Error at command: cpssPxSamplePortManagerApMandatoryParamsSet :%s", result)
                end
...
CLI_addCommand("interface", "802.3ap", {
  ...
  params = {
  {   type="named",
            { format="advertisement %ap_port_speed %ap_port_interface_mode %port_fec_mode_gen %port_fec_mode_gen1 ", name="advertisement", multiplevalues=true, help = "802.3ap advertised modes"},
