...
--********************************************************************************
--* remote_physical_ports_system_ingress_from_remote_port2.lua
--*
--* DESCRIPTION:
...
--*       traffic:
--*       flood from remote port that is 'trunk member' to other ports:
--*          (use next 'tags' permutations on ingress: 'untagged' , 'tag0 only' , 'tag1 only' , 'tag0+tag1' )
--*          a. not flood back to this port
--*          b. not flood back to other trunk member (BC3 native port)
...
local macSaPort2 = "000222222222"
local packetPayload = "6666"..-- dummy ethertype
        "000102030405060708090a0b0c0d0e0f"..
        "101112131415161718191a1b1c1d1e1f"..
        "202122232425262728292a2b2c2d2e2f"..
        "303132333435363738393a3b3c3d3e3f" 
local function basic_packet_flood_ingress_port_2(tagStr)
    if tagStr == nil then
        tagStr = ""
    end
    return macDaFlood .. macSaPort2 .. tagStr .. packetPayload
end
local vlanId = 1
...
local trunkId = 15
local function build_edsa_ingress_port_2(ingressTaggingType,tagStr)
    ...
    elseif ingressTaggingType == "tag0,tag1" then
        ...
        local_setFail("build_edsa_ingress_port_2 : unknown tag")
        return nil
    end
    ...
    if (rc ~=0) then
        local_setFail("build_edsa_ingress_port_2:" .. to_string(edsaStc))
        return
    end     
...
local eDsaStr_untagged  = build_edsa_ingress_port_2("untagged")
local eDsaStr_tag0      = build_edsa_ingress_port_2("tag0",nil)
local eDsaStr_tag1      = build_edsa_ingress_port_2("tag1",nil)
local eDsaStr_tag0_tag1 = build_edsa_ingress_port_2("tag0,tag1",tag0Str..tag1Str)
...
local function run_traffic_ingress_remote_physical_port_untagged()
    local transmitInfo = {
        devNum = devNum,
        portNum = port2,
        pktInfo = {fullPacket = basic_packet_flood_ingress_port_2(nil--[[untagged]])}
    }
    local egressInfoTable = {
        -- expected egress port to flood
        {portNum = port1, pktInfo = {fullPacket = basic_packet_flood_ingress_port_2(nil--[[untagged]])}}, -- untagged
        {portNum = port3, pktInfo = {fullPacket = basic_packet_flood_ingress_port_2(tag0Str_egress_for_ingress_untagged)}},-- defined 'Tag0,Tag1' but no 'Tag1' on this packet
        {portNum = port4, pktInfo = {fullPacket = basic_packet_flood_ingress_port_2(nil--[[untagged]])}},-- defined 'Tag1' but no 'Tag1' on this packet
        {portNum = port5, packetCount = 0 },--trunk member (same trunkId as ingress port)
        {portNum = port6, pktInfo = {fullPacket = basic_packet_flood_ingress_port_2(eDsaStr_untagged)}},--egress with eDSA
    }
...
local function run_traffic_ingress_remote_physical_port_tag0()
    local transmitInfo = {
        devNum = devNum,
        portNum = port2,
        pktInfo = {fullPacket = basic_packet_flood_ingress_port_2(tag0Str)}
    }
    local egressInfoTable = {
        -- expected egress port to flood
        {portNum = port1, pktInfo = {fullPacket = basic_packet_flood_ingress_port_2(nil--[[untagged]])}}, -- untagged
        {portNum = port3, pktInfo = {fullPacket = basic_packet_flood_ingress_port_2(tag0Str)}},-- defined 'Tag0,Tag1' but no 'Tag1' on this packet
        {portNum = port4, pktInfo = {fullPacket = basic_packet_flood_ingress_port_2(nil--[[untagged]])}},-- defined 'Tag1' but no 'Tag1' on this packet
        {portNum = port5, packetCount = 0 },--trunk member (same trunkId as ingress port)
        {portNum = port6, pktInfo = {fullPacket = basic_packet_flood_ingress_port_2(eDsaStr_tag0)}},--egress with eDSA
    }
    -- transmit packet and check that egress as expected
    rc = luaTgfTransmitPacketsWithExpectedEgressInfo(transmitInfo,egressInfoTable)
    if rc == 0 then
...
local function run_traffic_ingress_remote_physical_port_tag1()
    local transmitInfo = {
        devNum = devNum,
        portNum = port2,
        pktInfo = {fullPacket = basic_packet_flood_ingress_port_2(tag1Str)}
    }
    local egressInfoTable = {
        -- expected egress port to flood
        {portNum = port1, pktInfo = {fullPacket = basic_packet_flood_ingress_port_2(nil--[[untagged]])}}, -- untagged
        {portNum = port3, pktInfo = {fullPacket = basic_packet_flood_ingress_port_2(tag0Str_egress_for_ingress_untagged .. tag1Str)}},-- defined 'Tag0,Tag1'
        {portNum = port4, pktInfo = {fullPacket = basic_packet_flood_ingress_port_2(tag1Str)}},-- defined 'Tag1' 
        {portNum = port5, packetCount = 0 },--trunk member (same trunkId as ingress port)
        {portNum = port6, pktInfo = {fullPacket = basic_packet_flood_ingress_port_2(eDsaStr_tag1.. tag1Str)}},--egress with eDSA (with 'tag0' info) + outside the tag1 info
    }
    -- transmit packet and check that egress as expected
    rc = luaTgfTransmitPacketsWithExpectedEgressInfo(transmitInfo,egressInfoTable)
    if rc == 0 then
...
local function run_traffic_ingress_remote_physical_port_tag0_tag1()
    local transmitInfo = {
        devNum = devNum,
        portNum = port2,
        pktInfo = {fullPacket = basic_packet_flood_ingress_port_2(tag0Str..tag1Str)}
    }
    local egressInfoTable = {
        -- expected egress port to flood
        {portNum = port1, pktInfo = {fullPacket = basic_packet_flood_ingress_port_2(nil--[[untagged]])}}, -- untagged
        {portNum = port3, pktInfo = {fullPacket = basic_packet_flood_ingress_port_2(tag0Str..tag1Str)}},-- defined 'Tag0,Tag1'
        {portNum = port4, pktInfo = {fullPacket = basic_packet_flood_ingress_port_2(tag1Str)}},-- defined 'Tag1' 
        {portNum = port5, packetCount = 0 },--trunk member (same trunkId as ingress port)
        {portNum = port6, pktInfo = {fullPacket = basic_packet_flood_ingress_port_2(eDsaStr_tag0_tag1)}},--egress with eDSA
    }
    -- transmit packet and check that egress as expected
    rc = luaTgfTransmitPacketsWithExpectedEgressInfo(transmitInfo,egressInfoTable)
    if rc == 0 then
