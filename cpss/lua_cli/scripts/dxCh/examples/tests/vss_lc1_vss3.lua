...
--*       tests for 'lc1' in vss3.  (system described in examples/configurations/vss_lc1_vss3.txt)
--*
...
local port_peer_3   = port3 - 1 -- 17

-- define the ports needed for send/capture dsa tag packets on xcat devices 
local peersInfo = {{port4,port_peer_4},{port3,port_peer_3}}
-- call the engine about the 'peer ports'
luaTgfStatePhysicalConnectedPairs(devNum,peersInfo)

--set config
executeLocalConfig(luaTgfBuildConfigFileName("vss_lc1_vss3")) 
--run the tests for this device
util_vss_run_tests(devicesName_vss3_lc1)
--unset config
executeLocalConfig(luaTgfBuildConfigFileName("vss_lc1_vss3",true))
