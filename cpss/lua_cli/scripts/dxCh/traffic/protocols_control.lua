...
function createReceiverTask()
    ...
    if rcvTaskQueueID == nil then
        receiveQueue=luaMsgQCreate("receiveQueue", "RX_DATA_STC", 10)
    ...
    if rcvTaskID == nil then
        rcvTaskID = luaTaskCreate("RxTxTask","dxCh/traffic/receiverTask.lua")  -- the "RxTxTask" name is a reserved word
...
function signalReceiverTask()
    ...
    local proccessQ=luaMsgQAttach("receiveQueue", "RX_DATA_STC", luaGlobalGet("RxTxQueueID"))
...
function registerRx(name,ident,queueLen)
    ...
    local rxQ = luaMsgQCreate(name.."Queue", "RX_DATA_STC", queueLen)
...
---
--  do_checkProtocol
--        @description  A wrapper to check protocol handler in current
--                      Lua instance (not in receiver task)
--                      Will print received and sent packets,
--                      protocol match logs
--                      This function designed to be used manually!
--
--
--        @param name     - protocol name
--        @param proto    - protocol definition
--        @param timeout  - timeout to wait packet
--
function do_checkProtocol(name,proto,timeout)
    local protocols = {}
    protocols[name] = proto
    local rxQ = registerRx("protoChecker")
    local status, rxdata
    -- enable protocols_log()
    local protocols_log_enabled_saved = protocols_log_enabled
    protocols_log_enabled = true

    sendPacket_devport_saved = sendPacket_devport
    sendPacket_devport=function(devNum,portNum,packetData,minLen)
        print("sendPacket(devNum="..to_string(devNum)..
                ", portNum="..to_string(portNum)..
                ",data="..to_string(packetData)..
                ",minLen="..to_string(minLen)..")")
        return sendPacket_devport_saved(devNum,portNum,packetData,minLen)
    end

    status, rxdata = luaMsgQRecv(rxQ,timeout) --if second paramter is not a number endless wait
    if rxdata ~= nil then
        print("got data: "..to_string(rxdata))
        checkProtocol(protocols,rxdata)
    end
    sendPacket_devport = sendPacket_devport_saved
    protocols_log_enabled = protocols_log_enabled_saved
    unregisterRx("protoChecker",rxQ)
end
