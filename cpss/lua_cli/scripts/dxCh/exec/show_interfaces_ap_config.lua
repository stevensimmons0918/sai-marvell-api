...
-- ************************************************************************
---
--  show_interfaces_ap_config_port_range
...
local function show_interfaces_ap_config_port_range(params, command_data)
    ...
    "Ap Port Config\n" ..
    "--------------\n" ..
        "Interface   AP Enable    fcPause     fcAsmDir   fecSupport fecRequire noneceDisable   laneNum \n" ..
        "--------- ------------- ---------- ------------ ---------- ---------- ------------- ----------\n"
    footer_string1 = "\n"
    header_string2 =
        "\n" ..
    ...
    if true == command_data["status"] then
        ...
        for iterator, devNum, portNum in command_data:getPortIterator() do
            ...
            ret, localApEnablePtr = myGenWrapper("cpssDxChPortApPortConfigGet",{
            ...
            if (ret ~= GT_OK) then
                ...
                if(ret ~= 4) then
                    print("Error at show interfaces ap config command : cpssDxChPortApPortConfigGet " .. devNum .. "," .. portNum .. " :" ..  string.sub(returnCodes[ret],index + 1))
                end
            else
                ...
                if(localApEnablePtr~=nil and localApEnablePtr["apParamsPtr"]~=nil) then
                    ...
                    command_data["result"] =
                    string.format("%-10s%-13s%-11s%-13s%-11s%-11s%-14s%-11s",
                          devnum_portnum_string,
                          alignLeftToCenterStr(apEnable, 13),
...
-- ************************************************************************
---
--  show_interfaces_ap_config_single_port
...
--        @return       true on success, otherwise false and error message
--
local function show_interfaces_ap_config_single_port(params, command_data)
    -- Common variables declaration
    local devNum, portNum
...
--        @return       true on success, otherwise false and error message
--
local function show_interfaces_ap_config(params)
    local result, values
    ...
    if true == command_data:isOnePortInPortRange() then
        result, values = show_interfaces_ap_config_single_port(params, command_data)
    else
        result, values = show_interfaces_ap_config_port_range(params, command_data)
    end
...
CLI_addCommand("exec", "show interfaces ap config", {
    func = show_interfaces_ap_config,
