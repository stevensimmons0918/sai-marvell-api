...
--includes
cmdLuaCLI_registerCfunction("cpssDxChPortApPortStatusGet")
...
-- ************************************************************************
---
--  show_interfaces_ap_status_port_range
...
local function show_interfaces_ap_status_port_range(params, command_data)
    ...
    if true == command_data["status"] then
        ...
        for iterator, devNum, portNum in command_data:getPortIterator() do
            ...
            if (ret ~= GT_OK) then
                ...
                print("Error at show interfaces ap status command : cpssDxChPortApPortStatusGet " .. devNum .. "," .. portNum .. " :" ..  string.sub(returnCodes[ret],index + 1))
                do return end
            end
            
            if(localApParamsPtr["portMapShadowPtr"]["valid"] == true) then
                -- Traffic statistic getting.    
                ret, localApParamsPtr = myGenWrapper("cpssDxChPortApEnableGet",{
                        {"IN","GT_U8","devNum",devNum},
                        {"IN","GT_PHYSICAL_PORT_NUM","portNum",portNum},
                        {"OUT","GT_BOOL","apEnabled"}})

                if(localApParamsPtr["apEnabled"] == true) then
                    ret, localApParamsPtr = myGenWrapper("cpssDxChPortApPortStatusGet",{
                            {"IN","GT_U8","devNum",devNum},
                            {"IN","GT_PHYSICAL_PORT_NUM","portNum",portNum},
                            {"OUT","CPSS_DXCH_PORT_AP_STATUS_STC","apParamsPtr"}})
...
-- ************************************************************************
---
--  show_interfaces_ap_status_single_port
...
--        @return       true on success, otherwise false and error message
--
local function show_interfaces_ap_status_single_port(params, command_data)
    -- Common variables declaration
    local devNum, portNum
...
--        @return       true on success, otherwise false and error message
--
local function show_interfaces_ap_status(params)
    local result, values
    ...
    if true == command_data:isOnePortInPortRange() then
        result, values = show_interfaces_ap_status_single_port(params, command_data)
    else
        result, values = show_interfaces_ap_status_port_range(params, command_data)
    end
...
CLI_addCommand("exec", "show interfaces ap status", {
    func = show_interfaces_ap_status,
