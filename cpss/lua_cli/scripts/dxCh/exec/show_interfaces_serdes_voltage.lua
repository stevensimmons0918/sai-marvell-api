...
--********************************************************************************
--* show_interfaces_serdes_voltage.lua
--*
--* DESCRIPTION:
--*       display the Serdes Voltage
--*
--*
--********************************************************************************

local function Ret_Table_Update(Ret_Table, devNum, portNum, voltage)
    ...
    if (Ret_Table ~= nil ) then
    	...
    	Ret_Table[devNum][portNum].voltage = voltage
...
-- ************************************************************************
---
--  show_interfaces_serdes_voltage
--        @description  show interfaces Serdes Voltage
--
...
--
function show_interfaces_serdes_voltage(params) 
    ...
    if true == command_data["status"] then
        ...
        for iterator, devNum, portNum in command_data:getPortIterator() do
			...
			if( ret == GT_OK and numOfSerdesLanesPtr > 0) then
                ...
                command_data:clearLocalStatus()
				ret, values = myGenWrapper("cpssDxChPortDiagVoltageGet",{
				...
				if(ret ~= GT_OK) then
                    ...
                    --command_data:addError("Error cpssDxChPortDiagVoltageGet: device %d " ..
                    --                      "port %d: %s", devNum, portNum,
                    --                      portNum, returnCodes[ret])

                    print(devnum_portnum_string.." Error cpssDxChPortDiagVoltageGet ret: "..to_string(ret).." "..returnCodes[ret])
				else
					voltagePtr= to_string(values.voltagePtr)
                    ...
                    command_data["result"] = 
                        string.format("%-10s%-8s", 
                                      ...
                                      alignLeftToCenterStr(voltagePtr, 7))
    				command_data:addResultToResultArray()
    				command_data:updatePorts()

                    Ret_Table = Ret_Table_Update(Ret_Table, devNum, portNum,values.voltagePtr)
...
--------------------------------------------------------------------------------
-- command registration: show interfaces Serdes Voltage
--------------------------------------------------------------------------------

CLI_addCommand("exec", "show interfaces serdes voltage", {
    func = show_interfaces_serdes_voltage,
    help = "Interface(s) serdes voltage",
    params = {
		{	type= "named",
							"#all_interfaces",	   
