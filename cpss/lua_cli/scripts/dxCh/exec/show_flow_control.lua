...
local function flow_control_dump(params)
    ...
    enum={
            ...
            CPSS_DXCH_PORT_FC_MODE_LL_FC_E   = "LL FC"
        }
    enumState=  {
    ...
    for portNum = 0,maxPort do
        ...
        if does_port_exist(devNum, portNum) and portNum ~= 0x3f then
            ...
            if  0 == ret then
                ...
                if NOT_APPLICABLE == ret then
                    ...
                    fcEnable=errorMsg
                    errors=errors.."cpssDxChPortFlowControlEnableGet failed: "..returnCodes[ret].."\n"
                end
            end

            --Auto neg status and pause advertise
            ret,val = myGenWrapper("cpssDxChPortFlowCntrlAutoNegEnableGet",{
                {"IN","GT_U8","devNum",devNum},
                {"IN","GT_PHYSICAL_PORT_NUM","portNum",portNum},
            ...
            if  0 == ret then
                ...
                if NOT_APPLICABLE == ret then
                    ...
                    errors=errors.."cpssDxChPortFlowCntrlAutoNegEnableGet failed: "..returnCodes[ret].."\n"
                end
            end
            ...
            --Periodic Fc Status
            ret,val = myGenWrapper("cpssDxChPortPeriodicFcEnableGet",{
                {"IN","GT_U8","devNum",devNum},
                {"IN","GT_PHYSICAL_PORT_NUM","portNum",portNum},
            ...
            if  0 == ret then
                ...
                if NOT_APPLICABLE == ret then
                    ...
                    errors=errors.."cpssDxChPortPeriodicFcEnableGet failed: "..returnCodes[ret].."\n"
                end
            end

            --802.3x frames forwarding
            ret,val = myGenWrapper("cpssDxChPortForward802_3xEnableGet",{
                {"IN","GT_U8","devNum",devNum},
                {"IN","GT_PHYSICAL_PORT_NUM","portNum",portNum},
    ...
    enum=   {
            CPSS_DXCH_PORT_PERIODIC_FC_TYPE_GIG_E="GIG",
            CPSS_DXCH_PORT_PERIODIC_FC_TYPE_XG_E="XG"
            }
    dataToPrint=dataToPrint..("Periodic Flow Control interval:\n")
    dataToPrint=dataToPrint..("-------------------------------\n")
    for portType, name in pairs(enum) do
        ret,val = myGenWrapper("cpssDxChPortPeriodicFlowControlIntervalGet",{
            {"IN","GT_U8","devNum",devNum},
            {"IN","CPSS_DXCH_PORT_PERIODIC_FC_TYPE_ENT","portType",portType},
            {"OUT","GT_U32","valuePtr"}
            })
        if 0==ret then
            dataToPrint=dataToPrint..("Port type["..name.."]: "..val["valuePtr"].."\n")
        else
            if NOT_APPLICABLE == ret then
                dataToPrint=dataToPrint..(notApplicableMsg.."\n")
                break
            else
                errors=errors.."cpssDxChPortPeriodicFlowControlIntervalGet failed: "..returnCodes[ret].."\n"
            end
        end
...
CLI_addCommand("exec", "show flow-control ", {
  ...
  params={{type= "named", {format="device %devID", name="devID", help="ID of the device"},
          }
  }
})