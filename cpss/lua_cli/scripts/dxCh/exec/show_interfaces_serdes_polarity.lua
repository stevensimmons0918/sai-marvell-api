...
local function PrintLane(command_data,devNum,portNum,laneNum,Ret_Table)
    ...
    ret, val = myGenWrapper("cpssDxChPortSerdesPolarityGet",{
    ...
    if(ret~=GT_OK) then
        command_data:addError("Error cpssDxChPortSerdesPolarityGet: device %d " ..
...
-- Common variables declaration
    ...
    if true == command_data["status"] then
        ...
        for iterator, devNum, portNum in command_data:getPortIterator() do
            ...
            if ((laneNum == nil) or (laneNum == "all")) then
...
CLI_addCommand("exec", "show interfaces serdes polarity", {
    ...
    params = {
        {   type= "named",  {format="lane_num %LaneNumberOrAll", name="laneNum", help="Serdes lane number  or all"},
                            "#all_interfaces",     
