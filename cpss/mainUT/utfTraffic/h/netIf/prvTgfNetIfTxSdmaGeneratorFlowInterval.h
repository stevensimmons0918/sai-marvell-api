/*******************************************************************************
*              (c), Copyright 2001, Marvell International Ltd.                 *
* THIS CODE CONTAINS CONFIDENTIAL INFORMATION OF MARVELL SEMICONDUCTOR, INC.   *
* NO RIGHTS ARE GRANTED HEREIN UNDER ANY PATENT, MASK WORK RIGHT OR COPYRIGHT  *
* OF MARVELL OR ANY THIRD PARTY. MARVELL RESERVES THE RIGHT AT ITS SOLE        *
* DISCRETION TO REQUEST THAT THIS CODE BE IMMEDIATELY RETURNED TO MARVELL.     *
* THIS CODE IS PROVIDED "AS IS". MARVELL MAKES NO WARRANTIES, EXPRESSED,       *
* IMPLIED OR OTHERWISE, REGARDING ITS ACCURACY, COMPLETENESS OR PERFORMANCE.   *
********************************************************************************
*/
/**
********************************************************************************
* @file prvTgfNetIfTxSdmaGeneratorFlowInterval.h
*
* @brief Tx SDMA Generator rate adaptation for flow interval testing header file.
*
* @version   1
********************************************************************************
*/
#ifndef __prvTgfNetIfTxSdmaGeneratorFlowIntervalh
#define __prvTgfNetIfTxSdmaGeneratorFlowIntervalh

#ifdef __cplusplus
extern "C" {
#endif /* __cplusplus */

#include <common/tgfNetIfGen.h>

/**
* @internal prvTgfNetIfTxSdmaGeneratorFlowIntervalTest function
* @endinternal
*
* @brief   Tx SDMA generator rate adaptation for flow interval test.
*/
GT_VOID prvTgfNetIfTxSdmaGeneratorFlowIntervalTest
(
    GT_VOID
);


/**
* @internal prvTgfNetIfTxSdmaGeneratorFlowIntervalChangeTest function
* @endinternal
*
* @brief   Tx SDMA generator rate change for flow interval test.
*/
GT_VOID prvTgfNetIfTxSdmaGeneratorFlowIntervalChangeTest
(
    GT_VOID
);


#ifdef __cplusplus
}
#endif /* __cplusplus */

#endif /* __prvTgfNetIfTxSdmaGeneratorFlowIntervalh */



