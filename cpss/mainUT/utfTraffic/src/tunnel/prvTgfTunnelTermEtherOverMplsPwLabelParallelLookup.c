...
/**
********************************************************************************
* @file prvTgfTunnelTermEtherOverMplsPwLabelParallelLookup.c
*
...
/**
* @internal tgfTunnelTermEtherOverMplsPwLabelParallelLookupTtiConfigPerLookupSet function
* @endinternal
*
* @brief   build TTI Basic rule per lookup
*
* @retval GT_OK                    - on success
* @retval GT_HW_ERROR              - on HW error
* @retval GT_BAD_PARAM             - on wrong devNum
* @retval GT_BAD_PTR               - on NULL pointer
*/
static GT_VOID tgfTunnelTermEtherOverMplsPwLabelParallelLookupTtiConfigPerLookupSet
(
    GT_U8 lookupNum
)
{
    ...
    ttiAction.cwBasedPw                         = lookupInfoStc[lookupNum].cwBasedPw;
    ttiAction.applyNonDataCwCommand             = lookupInfoStc[lookupNum].applyNonDataCwCommand;
...
)
{
    ...
    UTF_VERIFY_EQUAL2_STRING_MAC(GT_OK, rc, "ERROR of tgfTrafficGeneratorPortTxEthCaptureSet: %d, &d\n",
                                 prvTgfDevNum, prvTgfPortsArray[PRV_TGF_EGR_2_PORT_IDX_CNS]);
    ...
    for (portIter = 0; portIter < PRV_TGF_PORT_COUNT_CNS; portIter++)
    {
        ...
        case PRV_TGF_EGR_2_PORT_IDX_CNS:
...
)
{
    ...
    for(lookupNum = 0; lookupNum < PRV_TGF_NUM_OF_TTI_LOOKUPS_CNS ; lookupNum++)
    {
        ...
        if((0 == lookupNum)&&(firstCall==GT_TRUE))
        {
            ...
            tgfTunnelTermEtherOverMplsPwLabelParallelLookupBridgeConfigSet(); /* basic test */

            /* Set TTI configuration */
            tgfTunnelTermEtherOverMplsPwLabelParallelLookupTtiConfigSet();
        ...
        lookupInfoStc[lookupNum].cwBasedPw              =   cwBasedPwArray[lookupNum];
        lookupInfoStc[lookupNum].applyNonDataCwCommand  =   applyNonDataCwCommandArray[lookupNum];

        tgfTunnelTermEtherOverMplsPwLabelParallelLookupTtiConfigPerLookupSet(lookupNum);
    }

    /* Generate traffic - expect forwarding */
    tgfTunnelTermEtherOverMplsPwLabelParallelLookupTrafficGenerate(0, GT_TRUE, GT_FALSE);

    /* Set the command for illegal control word to FORWARD */
    tgfTunnelTermEtherOverMplsPwLabelParallelLookupExceptionCommandSet(PRV_TGF_TTI_PW_CW_EXCEPTION_ILLEGAL_CONTROL_WORD_E, CPSS_PACKET_CMD_FORWARD_E);

    /* Generate traffic with illegal control word - expect forwarding */
    tgfTunnelTermEtherOverMplsPwLabelParallelLookupTrafficGenerate(1, GT_TRUE, GT_FALSE);

    /* Set the command for illegal control word to DROP_HARD */
    tgfTunnelTermEtherOverMplsPwLabelParallelLookupExceptionCommandSet(PRV_TGF_TTI_PW_CW_EXCEPTION_ILLEGAL_CONTROL_WORD_E, CPSS_PACKET_CMD_DROP_HARD_E);

    /* Generate traffic with illegal control word - expect dropping */
    tgfTunnelTermEtherOverMplsPwLabelParallelLookupTrafficGenerate(1, GT_FALSE, GT_FALSE);

    /* Set the command for illegal control word to TRAP */
    tgfTunnelTermEtherOverMplsPwLabelParallelLookupExceptionCommandSet(PRV_TGF_TTI_PW_CW_EXCEPTION_ILLEGAL_CONTROL_WORD_E, CPSS_PACKET_CMD_TRAP_TO_CPU_E);

    /* Generate traffic with illegal control word - expect trapping */
    tgfTunnelTermEtherOverMplsPwLabelParallelLookupTrafficGenerate(1, GT_FALSE, GT_TRUE);

    /* Set the base CPU code to 205 */
    tgfTunnelTermEtherOverMplsPwLabelParallelLookupCpuCodeBaseSet(205);
    ...
    if (restoreConfig==GT_TRUE)
    {
        ...
        tgfTunnelTermEtherOverMplsPwLabelParallelLookupConfigurationRestore();
