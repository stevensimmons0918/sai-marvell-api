...
/**
********************************************************************************
* @file prvTgfTunnelStartGeneric.c
*
...
#define PRV_TGF_TUNNEL_DATA1_COUNT_CNS    12
...
/* Tunnel etherType */
static GT_U32 prvTgfTunnelEtherType = 0x8899;

/* Tunnel data */
static GT_U32 prvTgfTunnelStartDataSize;
...
/* Tunnel data */
static GT_U8 prvTgfTunnelStartData[] = {
    0x30, 0x31, 0x32, 0x33, 0x34, 0x35, 0x36, 0x37, 0x38, 0x39, 0x40, 0x41,
    0x50, 0x51, 0x52, 0x53, 0x54, 0x55, 0x56, 0x57, 0x58, 0x59, 0x60, 0x61,
    0x62, 0x63, 0x64, 0x65, 0x66, 0x67, 0x68, 0x69, 0x70, 0x71, 0x72, 0x73
};
...
/**
* @internal prvTgfTunnelStartGenericLttRouteConfigurationSet function
* @endinternal
*
...
*                                       None
*/
static GT_VOID prvTgfTunnelStartGenericLttRouteConfigurationSet
(
    GT_U32      prvUtfVrfId
)
{
...
/**
* @internal prvTgfTunnelStartGenericPbrConfigurationSet function
* @endinternal
*
...
*                                       None
*/
static GT_VOID prvTgfTunnelStartGenericPbrConfigurationSet
(
    GT_U32      prvUtfVrfId
)
{
...
)
{
    ...
    switch(routingMode)
    {
        case PRV_TGF_IP_ROUTING_MODE_PCL_ACTION_E:
            ...
            prvTgfTunnelStartGenericPbrConfigurationSet(prvUtfVrfId);
            break;

        case PRV_TGF_IP_ROUTING_MODE_IP_LTT_ENTRY_E:

            prvTgfTunnelStartGenericLttRouteConfigurationSet(prvUtfVrfId);
            break;
...
)
{
    ...
    tunnelEntry.genCfg.vlanId           = PRV_TGF_NEXTHOPE_VLANID_CNS;
    tunnelEntry.genCfg.upMarkMode       = PRV_TGF_TUNNEL_START_MARK_FROM_ENTRY_E;
    tunnelEntry.genCfg.up               = 0;
    tunnelEntry.genCfg.etherType        = prvTgfTunnelEtherType;
    tunnelEntry.genCfg.genericType      = genType;
    tunnelEntry.genCfg.retainCrc        = GT_FALSE;

    /* tunnel MAC DA */
    cpssOsMemCpy(tunnelEntry.genCfg.macDa.arEther, prvTgfTunnelMacDa, sizeof(TGF_MAC_ADDR));

    /* tunnel start data*/
    cpssOsMemCpy(tunnelEntry.genCfg.data, prvTgfTunnelStartData, sizeof(prvTgfTunnelStartData));
    ...
    /* Check vlanId field */
    UTF_VERIFY_EQUAL1_STRING_MAC(PRV_TGF_NEXTHOPE_VLANID_CNS, tunnelEntry.genCfg.vlanId,
                                 "tunnelEntry.ipv4Cfg.vlanId: %d", tunnelEntry.genCfg.vlanId);

    /* Check macDa field */
    rc = cpssOsMemCmp(tunnelEntry.genCfg.macDa.arEther, prvTgfTunnelMacDa, sizeof(TGF_MAC_ADDR)) == 0 ?
        GT_OK : GT_FALSE;
    UTF_VERIFY_EQUAL1_STRING_MAC(GT_OK, rc, "tunnelEntry.genCfg.macDa.arEther[5]: 0x%2X",
                                 tunnelEntry.genCfg.macDa.arEther[5]);
}
...
)
{
    ...
    for (portIter = 0; portIter < PRV_TGF_PORT_COUNT_CNS; portIter++)
    {
        ...
        switch (portIter)
        {
            ...
            case PRV_TGF_NEXTHOPE_PORT_IDX_CNS:
                ...
                expectedPacketSize = (packetSize + prvTgfTunnelStartDataSize + TGF_CRC_LEN_CNS);
    ...
    /* check if etherType packet has the same MAC DA as prvTgfTunnelMacDa */
    cpssOsMemSet(&vfdArray[0], 0, sizeof(TGF_VFD_INFO_STC));
    vfdArray[0].mode = TGF_VFD_MODE_STATIC_E;
    vfdArray[0].offset = TGF_L2_HEADER_SIZE_CNS + TGF_VLAN_TAG_SIZE_CNS;
    vfdArray[0].cycleCount = TGF_ETHERTYPE_SIZE_CNS;
    vfdArray[0].patternPtr[0] = (GT_U8)(prvTgfTunnelEtherType >> 8);
    vfdArray[0].patternPtr[1] = (GT_U8)(prvTgfTunnelEtherType);
    ...
    UTF_VERIFY_EQUAL1_STRING_MAC(1, numTriggers,
            ...
            prvTgfTunnelEtherType);

    if (prvTgfTunnelStartDataSize)
    {
        /* check if tunnel data inserted after L2 header */
        cpssOsMemSet(&vfdArray[0], 0, sizeof(TGF_VFD_INFO_STC));
        vfdArray[0].mode = TGF_VFD_MODE_STATIC_E;
        vfdArray[0].offset = TGF_L2_HEADER_SIZE_CNS + TGF_VLAN_TAG_SIZE_CNS + TGF_ETHERTYPE_SIZE_CNS;
        vfdArray[0].cycleCount = PRV_TGF_TUNNEL_DATA1_COUNT_CNS;
        cpssOsMemCpy(vfdArray[0].patternPtr, prvTgfTunnelStartData, PRV_TGF_TUNNEL_DATA1_COUNT_CNS);
    ...
    /* check L3 header offset */
    cpssOsMemSet(&vfdArray[0], 0, sizeof(TGF_VFD_INFO_STC));
    vfdArray[0].mode = TGF_VFD_MODE_STATIC_E;
    vfdArray[0].offset = TGF_L2_HEADER_SIZE_CNS + TGF_VLAN_TAG_SIZE_CNS + TGF_ETHERTYPE_SIZE_CNS + prvTgfTunnelStartDataSize;
    vfdArray[0].cycleCount = 1;
    vfdArray[0].patternPtr[0] = 0x45;
    ...
    UTF_VERIFY_EQUAL1_STRING_MAC(1, numTriggers,
            "\n  L3 should start from %d byte", vfdArray[0].offset);
