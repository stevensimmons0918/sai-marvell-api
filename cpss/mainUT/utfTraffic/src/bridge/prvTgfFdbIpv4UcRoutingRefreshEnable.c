...
/**
********************************************************************************
* @file prvTgfFdbIpv4UcRoutingRefreshEnable.c
*
* @brief FDB IPv4 UC Routing + Refresh configurations
*
* @version   3
********************************************************************************
*/
...
static GT_U32   prv_tgf_nexthop_2_port_idx_cns = 3;
/* nextHop port number to receive traffic from */
#define PRV_TGF_NEXTHOP_2_PORT_IDX_CNS    prv_tgf_nexthop_2_port_idx_cns
...
static GT_BOOL prvTgfUcIpv4RouteRefreshStatusFirstCall=GT_TRUE;/* is this the first call to sav ethe value */
static GT_BOOL prvTgfUcIpv4RouteRefreshStatusGet;
...
)
{
    ...
    /* AUTODOC: create VLAN 6 with tagged ports 1,2,3,4 */
    ...
    PRV_TGF_NEXTHOP_2_PORT_IDX_CNS = nexthop_2_port_idx;
...
)
{
    ...
    if(prvTgfUcIpv4RouteRefreshStatusFirstCall==GT_TRUE)
    {
        /* save fdb routing uc refresh status*/
        rc =  prvTgfBrgFdbRoutingUcRefreshEnableGet(prvTgfDevNum,&prvTgfUcIpv4RouteRefreshStatusGet);
...
)
{
    ...
    for (portIter = 0; portIter < portsCount; portIter++)
    {
        ...
        if((prvTgfPortsArray[portIter]==sendPortNum)||
            ((expectTraffic==GT_TRUE) && (portIter == PRV_TGF_NEXTHOP_2_PORT_IDX_CNS)))
        {
...
)
{
    ...
    rc =  prvTgfBrgFdbRoutingUcRefreshEnableSet(prvTgfDevNum,prvTgfUcIpv4RouteRefreshStatusGet);
    UTF_VERIFY_EQUAL1_STRING_MAC(GT_OK, rc, "prvTgfBrgFdbRoutingUcRefreshEnableSet: %d", prvTgfDevNum);

    prvTgfUcIpv4RouteRefreshStatusFirstCall=GT_TRUE;
