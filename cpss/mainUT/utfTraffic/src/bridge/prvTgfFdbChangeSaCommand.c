...
/**
********************************************************************************
* @file prvTgfFdbChangeSaCommand.c
*
* @brief Change SA Command. SA command can be defined as Drop,
...
/* change SA command tests sequence */
static PRV_TGF_PACKET_CMD_ENT  prvTgfSaCommandTestSequence[] =
{
...
#define PRV_TGF_SEQUENCE_LEN_CNS   \
    sizeof(prvTgfSaCommandTestSequence)/sizeof(prvTgfSaCommandTestSequence[0])

/******************************* Test packet **********************************/

static TGF_MAC_ADDR prvTgfMacAddr =
{
...
/**
* @internal prvTgfFdbChangeSaCommandConfigSet function
* @endinternal
*
* @brief   Set test configuration
*/
GT_VOID prvTgfFdbChangeSaCommandConfigSet
(
    GT_VOID
)
{
...
/**
* @internal prvTgfFdbChangeSaCommandTrafficGenerate function
* @endinternal
*
* @brief   Generate traffic
*/
GT_VOID prvTgfFdbChangeSaCommandTrafficGenerate
(
    GT_VOID
)
{
    ...
    if (PRV_CPSS_SIP_6_CHECK_MAC(prvTgfDevNum))
    {
        ...
        if(PRV_CPSS_SIP_6_CHECK_MAC(prvTgfDevNum))
        {
            ...
            rc = cpssDxChBrgFdbSaDaCmdDropModeSet(
                prvTgfDevNum, CPSS_MAC_TABLE_DROP_E, restoreSip6FdbDaDropCommand);
    ...
    {
        /* print log message */
        switch (prvTgfSaCommandTestSequence[stageNum])
        {
            case PRV_TGF_PACKET_CMD_DROP_HARD_E:
                PRV_UTF_LOG0_MAC("\n========== Traffic generate to test SA command <DROP_HARD> =============\n\n");
                break;

            case PRV_TGF_PACKET_CMD_FORWARD_E:
                PRV_UTF_LOG0_MAC("\n========== Traffic generate to test SA command <FORWARD> ===============\n\n");
                break;

            case PRV_TGF_PACKET_CMD_MIRROR_TO_CPU_E:
                if(PRV_CPSS_SIP_6_CHECK_MAC(prvTgfDevNum))
                {
                    /* Not supported SA command at SIP-6 FDB */
                    continue;
                }
                PRV_UTF_LOG0_MAC("\n========== Traffic generate to test SA command <MIRROR_TO_CPU> =========\n\n");
                break;
            ...
            case PRV_TGF_PACKET_CMD_TRAP_TO_CPU_E:
                ...
                PRV_UTF_LOG0_MAC("\n========== Traffic generate to test SA command <TRAP_TO_CPU> ===========\n\n");
                break;

            default:
                rc = GT_BAD_PARAM;
                UTF_VERIFY_EQUAL1_STRING_MAC(GT_OK, rc, "Illegal SA command: %d",
                                             prvTgfSaCommandTestSequence[stageNum]);
        }
        ...
        cpssOsMemCpy(macEntry.key.key.macVlan.macAddr.arEther, prvTgfMacAddr, sizeof(TGF_MAC_ADDR));

        /* select SA command from test sequence list */
        macEntry.saCommand                = prvTgfSaCommandTestSequence[stageNum];

        /* AUTODOC: add 4 FDB entries with MACs 00:00:00:00:00:02 and: */
        /* AUTODOC:   1. SA cmd=DROP_HARD, VLAN 2, port 1 */
        /* AUTODOC:   2. SA cmd=FORWARD, VLAN 3, port 2 */
        /* AUTODOC:   3. SA cmd=MIRROR_TO_CPU, VLAN 4, port 2 */
        /* AUTODOC:   4. SA cmd=TRAP_TO_CPU, VLAN 5, port 1 */
        ...
        /* AUTODOC:   DA=00:00:00:00:00:01, SA=00:00:00:00:00:02 */
        /* AUTODOC:   VID=[2,3,4,5] */
        ...
        /* AUTODOC:   2 packet - captured on ports 1,2,3 */
        /* AUTODOC:   3 packet - captured on ports 1,2,3 */
...
/**
* @internal prvTgfFdbChangeSaCommandConfigRestore function
* @endinternal
*
* @brief   Restore test configuration
*/
GT_VOID prvTgfFdbChangeSaCommandConfigRestore
(
    GT_VOID
)
{
    ...
    /* invalidate vlan entries (and reset vlan entries) */
    for (stageNum = 0; stageNum < PRV_TGF_SEQUENCE_LEN_CNS; stageNum++)
    {
