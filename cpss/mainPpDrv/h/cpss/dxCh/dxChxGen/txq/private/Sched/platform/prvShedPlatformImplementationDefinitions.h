...
struct cpssSchedEnv
{
	...
	GT_U8	devNumber;
	GT_U8	burstMode;
    GT_U32  globalAddress;
    GT_U32  pdqNumber;

	struct PRV_CPSS_SCHED_TREE_MAX_SIZES_STC		tree_structure;
};


#define DECLARE_CPSS_ENV(env)		struct cpssSchedEnv * env;
#define ASSIGN_CPSS_ENV(env,hndl)	env = (struct cpssSchedEnv *)hndl;
...
#define CPSS_ENV(env,hndl, NULL_PTR_ERROR, WRONG_MAGIC_ERROR)	\
	struct cpssSchedEnv * env = (struct cpssSchedEnv *)hndl;\
	CHECK_HENV_PTR(env, NULL_PTR_ERROR, WRONG_MAGIC_ERROR)

/* function below creates new cpssSchedEnv handle */
...
#define PRV_SCHED_TILE_NUM_GET(_ctl) (((struct cpssSchedEnv *)(TM_ENV(_ctl)))->pdqNumber)
