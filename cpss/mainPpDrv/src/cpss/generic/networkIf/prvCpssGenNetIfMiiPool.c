...
/**
********************************************************************************
* @file prvCpssGenNetIfMiiPool.c
*
* @brief Ethernet port buffer management module. These pools are used by the
 ...
 *
 * Fields:
 *      orig_poolBlock_PTR      - Pointer to the allocated memory block -- needed for free purposes.
...
)
{
    ...
    if (NULL == poolPtr)
    {
        ...
        head_PTR->orig_poolBlock_PTR = NULL;/* application responsible ... with proper 'free' function */
    }
    else
    {
        neededPoolSize = poolSize;
        head_PTR->orig_poolBlock_PTR = poolPtr;
    }
...
#ifdef ETH_POOL_DEBUG
/**
* @internal prvCpssGenNetIfMiiPoolPrintfInfo function
* @endinternal
*
* @brief   Helper function to print all pool (rx/tx) information
*
* @note   APPLICABLE DEVICES:      xCat3; AC5; Lion2; Bobcat2; Caelum; Aldrin; AC3X; Aldrin2; Bobcat3; Falcon; AC5P; AC5X; Harrier; Ironman.
* @note   NOT APPLICABLE DEVICES:  None.
*
...
GT_STATUS prvCpssGenNetIfMiiFreeMemory
(
    IN      PRV_CPSS_GEN_NETIF_MII_POOL_TYPE_ENT    poolIdx,
...
)
{
    ...
    head_PTR = &poolArrPtr[poolIdx];
#endif
    if(head_PTR->orig_poolBlock_PTR)
    {
#ifdef OS_MALLOC_MEMORY_LEAKAGE_DBG
        freeFunc(head_PTR->orig_poolBlock_PTR,__FILE__,__LINE__);
#else
        freeFunc(head_PTR->orig_poolBlock_PTR);
#endif /*OS_MALLOC_MEMORY_LEAKAGE_DBG*/
    }
    head_PTR->orig_poolBlock_PTR = NULL;
