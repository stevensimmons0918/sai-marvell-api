...
GT_U32 mvHwsDdr3TipMaxCSGet(GT_U32 devNum)
{
  ...
  if (!max_cs)
  {
      ...
      if ((busId >= MAX_BUS_NUM) || (IS_BUS_ACTIVE(topologyMap->activeBusMask, busId) == GT_FALSE))
          return max_cs;
...
)
{
	...
	*csMask = sameBusCs = CS_BIT_MASK;
	...
	for (busCnt=0; busCnt<octetsPerInterfaceNum; busCnt++)
	{
		...
		allBusCs |= (GT_U32)(topologyMap->interfaceParams[interfaceId].asBusParams[busCnt].csBitmask);
		sameBusCs &= (GT_U32)(topologyMap->interfaceParams[interfaceId].asBusParams[busCnt].csBitmask);
