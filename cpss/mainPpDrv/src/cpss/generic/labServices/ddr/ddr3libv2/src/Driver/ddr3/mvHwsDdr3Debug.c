...
)
{
    ...
    cpssOsMemCpy(&configFuncInfo[devNum], configFunc, sizeof(MV_HWS_TIP_CONFIG_FUNC_DB));
...
)
{
   ...
   cpssOsMemCpy(result, trainingResult, (sizeof(MV_HWS_RESULT ) * MAX_STAGE_LIMIT * MAX_INTERFACE_NUM));
   return GT_OK;
}
...
{
    cpssOsMemCpy(&xsbInfo[devNum], xsbInfoTable, sizeof(MV_HWS_XSB_INFO));
    return GT_OK;
}
...
(
...
)
 {
	...
	*ptr = NULL;
    ...
    case 0:
        topologyMap->interfaceActiveMask = (GT_U8)value;
        break;
    ...
    case 0x4:
        genericInitController = (GT_U8)value;
        break;
   ...
   case 0x21:
       isFreqOld = (GT_U8)value;
       break;

   case 0x23:
       isDfsDisabled = (GT_U8)value;
       break;

   case 0x24:
       isPllBeforeInit = (GT_U8)value;
       break;

   case 0x25:
       isAdllCalibBeforeInit = (GT_U8)value;
       break;
...
#endif
   case 0x28:
       isTuneResult = (GT_U8)value;
       break;

   case 0x29:
       isValidateWindowPerIf = (GT_U8)value;
       break;

   case 0x2a:
       isValidateWindowPerPup = (GT_U8)value;
       break;

   case 0x30:
       sweepCnt = (GT_U8)value;
       break;

   case 0x31:
       isBistResetBit = (GT_U8)value;
       break;

   case 0x32:
       isDfsInInit = (GT_U8)value;
       break;
   case 0x33:
       *ptr = &gZnodtData;
       break;
...
*/
   ...
   case 0x5B:
       *ptr = &gZpriCtrl;
       break;
   case 0x5C:
       isRegDump = (GT_U8)value;
       break;

   case 0x5D:
       *ptr = &vrefInitialValue;
       break;
   ...
   case 0x60:
       *ptr = &xsbValidationBaseAddress;
       break;

   case 0x67:
       *ptr = &activateSelectBeforeRunAlg;
       break;

   case 0x68:
       *ptr = &activateDeselectAfterRunAlg;
       break;
   ...
   case 0x112:
    ...
    default:
        ...
        {
            interfaceId = (flagId - 0x200) % MAX_INTERFACE_NUM;
            topologyMap->interfaceParams[interfaceId].memoryFreq = (MV_HWS_DDR_FREQ)value;
        }
        else if ((flagId >=0x210) && (flagId < 0x220))
        {
            interfaceId = (flagId - 0x210) % MAX_INTERFACE_NUM;
            topologyMap->interfaceParams[interfaceId].speedBinIndex = (MV_HWS_SPEED_BIN)value;
        }
        else if ((flagId >=0x220) && (flagId < 0x230))
        {
            interfaceId = (flagId - 0x220) % MAX_INTERFACE_NUM;
            topologyMap->interfaceParams[interfaceId].busWidth = (MV_HWS_BUS_WIDTH)value;
        }
        else if ((flagId >=0x230) && (flagId < 0x240))
        {
            interfaceId = (flagId - 0x230) % MAX_INTERFACE_NUM;
            topologyMap->interfaceParams[interfaceId].memorySize = (MV_HWS_MEM_SIZE)value;
        }
        else if ((flagId >=0x240) && (flagId < 0x250))
        {
            interfaceId = (flagId - 0x240) % MAX_INTERFACE_NUM;
            topologyMap->interfaceParams[interfaceId].casL = (GT_U8)value;
        }
        else if ((flagId >=0x250) && (flagId < 0x260))
        {
            interfaceId = (flagId - 0x250) % MAX_INTERFACE_NUM;
            topologyMap->interfaceParams[interfaceId].casWL = (GT_U8)value;
        }
        else if ((flagId >=0x270) && (flagId < 0x2cf))
        {
            interfaceId = (flagId - 0x270)/MAX_BUS_NUM;
			pupId = (flagId - 0x270)%MAX_BUS_NUM;
            topologyMap->interfaceParams[interfaceId].asBusParams[pupId].isCkSwap = (GT_BOOL)value;
        }
        else if ((flagId >=0x2d0) && (flagId < 0x32f))
        {
            interfaceId = (flagId - 0x2d0)/MAX_BUS_NUM;
			pupId = (flagId - 0x2d0)%MAX_BUS_NUM;
            topologyMap->interfaceParams[interfaceId].asBusParams[pupId].isDqsSwap = (GT_BOOL)value;
        }
        else if ((flagId >=0x330) && (flagId < 0x38f))
        {
            interfaceId = (flagId - 0x330)/MAX_BUS_NUM;
			pupId = (flagId - 0x330)%MAX_BUS_NUM;
            topologyMap->interfaceParams[interfaceId].asBusParams[pupId].csBitmask = (GT_U8)value;
        }
        else if ((flagId >=0x390) && (flagId < 0x3ef))
        {
            interfaceId = (flagId - 0x390)/MAX_BUS_NUM;
			pupId = (flagId - 0x390)%MAX_BUS_NUM;
            topologyMap->interfaceParams[interfaceId].asBusParams[pupId].mirrorEnableBitmask = (GT_BOOL)value;
        }
