...
/**
********************************************************************************
* @file prvCpssDxChLpmTcamEntriesHandler.c
*
* @brief This file contains code for the object to handle TCAM entries.
...
)
{
    ...
    if (((ipTcamShadowPtr->shadowType == PRV_CPSS_DXCH_LPM_TCAM_XCAT_SHADOW_E) && (partitionEnable == GT_TRUE)) ||
    ...
    else if ((ipTcamShadowPtr->shadowType == PRV_CPSS_DXCH_LPM_TCAM_XCAT_SHADOW_E) && (partitionEnable == GT_FALSE))
    {
        ...
        for (i = 0 ; i < PRV_CPSS_DXCH_LPM_TCAM_MGM_POOL_NUM_OF_TCAMS_CNS; i++)
        {
            for (j = 0 ; j < PRV_CPSS_DXCH_LPM_TCAM_MGM_POOL_NUM_OF_PROTOCOLS_CNS; j++)
            {
                if (ipTcamShadowPtr->tcamEntriesHandler->ipTcamPool.tcamMgm[i][j] != NULL)
                {
                    ...
                    /* loop on the other pointers and remove those that with the same value */
                    for (new_i = i ; new_i < PRV_CPSS_DXCH_LPM_TCAM_MGM_POOL_NUM_OF_TCAMS_CNS; new_i++)
                    {
                        for (new_j = 0 ; new_j < PRV_CPSS_DXCH_LPM_TCAM_MGM_POOL_NUM_OF_PROTOCOLS_CNS; new_j++)
                        {
                            if(i == new_i && j == new_j)
                            {
                                /* skip 'own' pointer */
                            ...
                            if(ipTcamShadowPtr->tcamEntriesHandler->ipTcamPool.tcamMgm[i][j] ==
                               ipTcamShadowPtr->tcamEntriesHandler->ipTcamPool.tcamMgm[new_i][new_j])
                            {
                                /* share the same pointer */
                                /* but we already free all it's related memory when called prvCpssDxChLpmTcamMgmDestroy(...) */
                                ipTcamShadowPtr->tcamEntriesHandler->ipTcamPool.tcamMgm[new_i][new_j] = NULL;
                            }
                        }
                    }

                    /*set own pointer to NULL */
...
/**
* @internal createIpTcamMgmPools function
* @endinternal
*
* @brief   In partition mode, the TCAM entries handler is used as a glue layer to
*         the common tcam pool manager code which implements a partitioned TCAM.
*         This function divides the TCAM range to the various entry types and
*         creates pool managers for each of those types.
* @param[in] ipTcamShadowPtr          - the shadow to alloc the pools for.
* @param[in] indexesRangePtr          - the range of TCAM indexes allocated for this TCAM
*                                      entries handler
* @param[in] partitionEnable          - GT_TRUE:  partition the TCAM range according to the
* @param[in] capacityCfgPtr           any unused TCAM entries will
*                                      be allocated to IPv4 UC entries
...
*                                       GT_FAIL otherwise.
*
* @note Since this function creates separate pools for each entry type, it can't
...
/**
* @internal ipTcamMemPoolCheetahInit function
* @endinternal
*
* @brief   This function initializes the managment pool for Cheetah+
*         and for xCat that configured to work in policy based routing.
* @param[in] tcamStartIndex           -  first TCAM line allocated to ip
* @param[in] tcamEndIndex             -  last TCAM line allocated to ip
* @param[in] numOfIpPrefixes          -  total number of ip prefixes to manage (Ipv4+Ipv6)
* @param[in] ipTcamShadowPtr          -  the shadow to alloc the pools for
*
* @retval GT_OK                    - on success, or
* @retval GT_OUT_OF_CPU_MEM        - if unable to create pool.
*/
static GT_STATUS ipTcamMemPoolCheetahInit
(
    IN GT_U32                                           tcamStartIndex,
...
/**
* @internal ipTcamMemPoolRouterTcamInit function
* @endinternal
*
* @brief   This function initializes the managment pool for Cheetah2 in Router Tcam.
*
* @param[in] tcamStartIndex           -  first TCAM line allocated to ip
* @param[in] tcamEndIndex             -  last TCAM line allocated to ip
* @param[in] numOfIpv4Prefixes        -  total number of IPv4 prefixes to manage
* @param[in] numOfIpv6Prefixes        -  total number of IPv4 prefixes to manage
* @param[in] numOfMcSrcAddr           -  total number of Mc source addresses to manage
* @param[in] protocolStack            -  the protocol stack to support
* @param[in] ipTcamShadowPtr          -  the shadow to alloc the pools for
* @param[in] numOfReservedPrefixes    - num of entries (single entries) which must be
*                                      reserveed for external management (for example
*                                      PCL redirecting to LTT)
*                                      Relevant if freeIndexesPtr != NULL
*
* @param[out] freeIndexesPtr           - if != NULL then used to return reserved indexes
*                                      requested in numOfReservedPrefixes
*
* @retval GT_OK                    - on success, or
* @retval GT_NO_RESOURCE           - if number of prefixes exceeds number of indexes in table
* @retval GT_OUT_OF_CPU_MEM        - if unable to create pool.
*/
static GT_STATUS ipTcamMemPoolRouterTcamInit
(
...
)
{
        ...
        (numOfReservedPrefixes > 0))
    {
        ...
        for (i = 0 ; i < columns ; i++)
        {
            ...
            segStartIdx = i * PRV_SHARED_IP_LPM_DIR_LPM_TCAM_COMMON_SRC_GLOBAL_VAR_GET(tcamRows) + tcamStartIndex;
                ...
                if (indexesLeftInColumn == 0)
                {
                    if (i != 0)
                    {
                        freeIndexesPtr->segment[reservedSegNum].startIndex   =
                            ipv6IdxSegments.segment[0].startIndex + i * PRV_SHARED_IP_LPM_DIR_LPM_TCAM_COMMON_SRC_GLOBAL_VAR_GET(tcamRows);
                        freeIndexesPtr->segment[reservedSegNum].endIndex     =
                            ipv6IdxSegments.segment[0].endIndex + i * PRV_SHARED_IP_LPM_DIR_LPM_TCAM_COMMON_SRC_GLOBAL_VAR_GET(tcamRows);
...
/**
* @internal ipTcamMemPoolXcatInit function
* @endinternal
*
* @brief   This function initializes the managment pool for Xcat in Router Tcam.
*
* @param[in] tcamStartIndex           -  first TCAM line allocated to ip
* @param[in] tcamEndIndex             -  last TCAM line allocated to ip
* @param[in] numOfIpv4Prefixes        -  total number of IPv4 prefixes to manage
...
/**
* @internal tcamHandlerGetMemPoolPtr function
* @endinternal
*
* @brief   In partition mode, the TCAM entries handler is used as a glue layer to
*         the common tcam pool manager code which implements a partitioned TCAM.
...
* @param[in] entryType                - entry type
*
* @param[out] memPoolPtrPtr            - memory pool used by TCAM entries handler for
*                                      this entry type
*
* @retval GT_OK                    - on success
*/
...
/**
* @internal prvCpssDxCh3TcamEntriesHandlerCreate function
* @endinternal
*
* @brief   The TCAM entries handler is responsible for handling the TCAM entries
*         and deciding which entries to allocate to each prefix type.
*         The TCAM entries handler has two modes of operation:
*         - Partition mode. In this mode the TCAM range allocated for the TCAM
*         entries handler in partitioned so each entry type has fixed section
...
* @param[in] capacityCfgPtr           - capacity configuration
* @param[in] protocolStack            - the protocol stack to support
* @param[in] tcamLines                - total number of lines in the TCAM
* @param[in,out] ipTcamShadowPtr          - the shadow type to create the TCAM entries handler for
*                                       GT_OK on success, or
*                                       GT_FAIL otherwise.
*
* @note Supported only for Cheetah3 devices with no partition mode.
*
*/
static GT_STATUS prvCpssDxCh3TcamEntriesHandlerCreate
(
...
/**
* @internal tcamHandlerAllocate function
* @endinternal
*
* @brief   Allocates TCAM entry for specific type of prefix. The entry is allocated
*         in a range that is bounded by two other entries (bounded in the meaning
...
/**
* @internal tcamHandlerAllocateEntry function
* @endinternal
*
* @brief   Allocates single TCAM entry. The entry is allocated in a range that is
...
/**
* @internal tcamHandlerAllocateLine function
* @endinternal
*
* @brief   Allocates line TCAM entry. The line is allocated in a range that is
...
/**
* @internal tcamHandlerFreeLine function
* @endinternal
*
* @brief   Free a line by moving the used entries in this line away from the line.
*
* @param[in] ipTcamShadowPtr          - the shadow pointer we are working on
* @param[in] lineIndexToFree          - index of the line to free
*
* @retval GT_OK                    - if allocation succedded
* @retval GT_OUT_OF_RANGE          - if the indexes were out of range
* @retval GT_NOT_APPLICABLE_DEVICE - on not applicable device
...
/**
* @internal tcamHandlerSetEntryUsage function
* @endinternal
*
* @brief   Mark entry as used/free.
*
* @param[in] tcamHandlerPtr           - points to TCAM entries handler
* @param[in] entry                    -  to mark
* @param[in] usageState               - GT_TRUE:  mark as used
...
/**
* @internal tcamHandlerSetLineUsage function
* @endinternal
*
* @brief   Mark entire line as used/free.
*
* @param[in] tcamHandlerPtr           - points to TCAM entries handler
* @param[in] line                     -  to mark
* @param[in] usageState               - GT_TRUE:  mark as used
*                                      GT_FALSE: mark as free
*
* @retval GT_OK                    - if allocation succedded
...
/**
* @internal tcamHandlerAllocByGivenRule function
* @endinternal
*
* @brief   Allocates a rule by a another given rule.
*
...
/**
* @internal tcamHandlerFree function
* @endinternal
*
* @brief   Frees a TCAM entry.
*
...
/**
* @internal tcamHandlerCheckAvailbleMem function
* @endinternal
*
* @brief   Checks whether the TCAM has enough available space to add additional
*         entries of specific entry type.
...
/**
* @internal tcamHandlerGetRuleIdx function
* @endinternal
*
* @brief   The TCAM entries handler maintains internal array that holds rules (those
*         rules are also associated with the prefixes attached to them).
...
/**
* @internal tcamHandlerGetRuleByRuleIdx function
* @endinternal
*
* @brief   The TCAM entries handler maintains internal array that holds rules (those
*         rules are also associated with the prefixes attached to them).
...
/**
* @internal tcamHandlerSetRealIdxAsAlloc function
* @endinternal
*
* @brief   Marks specific TCAM entry as used and returns the rule associated with
*         this TCAM entry.
...
/**
* @internal convertOffsetToIndex function
* @endinternal
*
* @brief   Converts offset within the entire TCAM to index within the TCAM section
*         managed by TCAM entries handler.
* @param[in] tcamHandlerPtr           - points to TCAM entries handler
* @param[in] offset                   -  in the entire TCAM, ordered column by column
* @param[in] isEntry                  - GT_TRUE:  converting entry offset
*                                      GT_FALSE: converting line offset
*
* @param[out] indexPtr                 - points to index within TCAM section managed by TCAM
*                                      entries handler
*
* @retval GT_OK                    - on success
* @retval GT_FAIL                  - on error
* @retval GT_OUT_OF_RANGE          - if offset is out of the range allocated to
...
/**
* @internal convertIndexToOffset function
* @endinternal
*
* @brief   Converts index within the TCAM section managed by TCAM entries handler
*         to offset within the entire TCAM.
* @param[in] tcamHandlerPtr           - points to TCAM entries handler
* @param[in] index                    -  within the TCAM section managed by TCAM
*                                      entries handler
* @param[in] isEntry                  - GT_TRUE:  converting entry index
*                                      GT_FALSE: converting line index
*
* @param[out] offsetPtr                - points to offset within the entire TCAM, ordered
*                                      column by column
*
* @retval GT_OK                    - on success
* @retval GT_FAIL                  - on error
...
/**
* @internal tcamHandlerUpdatePrefixesAllocated function
* @endinternal
*
* @brief   Update number of prefixes allocated in the TCAM handler.
*
* @param[in] tcamHandlerPtr           - points to TCAM entries handler
* @param[in] entryType                - entry type
* @param[in] amount                   - number of entries
* @param[in] increase                 - GT_TRUE:   number of allocated prefixes
