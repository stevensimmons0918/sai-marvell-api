...
/**
********************************************************************************
* @file prvCpssDxChLpmTcamPatTrie.c
*
...
#define PRV_SHARED_IP_LPM_DIR_LPM_TCAM_PAT_TRIE_SRC_GLOBAL_VAR_GET(_var)\
    PRV_SHARED_GLOBAL_VAR_GET(mainPpDrvMod.ipLpmDir.lpmTcamPatTrieSrc._var)
...
typedef struct PRV_CPSS_DXCH_TRAVERSE_DEL_FUN_EXTRA_DATA_STCT
{
    ...
    IN GT_MEMFREEFUNCPTR                                      freeFunc;
}PRV_CPSS_DXCH_TRAVERSE_DEL_FUN_EXTRA_DATA_STC;

/**
* @internal getIpBit function
* @endinternal
*
* @brief   returns the n- bit of an Ip GT_U8 array address
*
* @note   APPLICABLE DEVICES:      xCat3; AC5.
* @note   NOT APPLICABLE DEVICES:  None.
*
* @param[in] ipAddrPtr                - ip address.
* @param[in] bitIndex                 - the bit index.
...
/**
* @internal setNonValidRuleNodeBoundingSon function
* @endinternal
*
* @brief   set the pointer to valid node bounding son's rule under this no-valid
*         rule. works up until it reaches a valid node.
*
* @note   APPLICABLE DEVICES:      xCat3; AC5.
* @note   NOT APPLICABLE DEVICES:  None.
*
* @param[in] trieNodePtr              - the trie node to start working up from.
* @param[in] ipTcamShadowPtr          - the ip shadow we are working on.
* @param[in] indicateUpdateChildren   - in the case of bulk insert, whether to
...
/**
* @internal recordOldNode function
* @endinternal
*
* @brief   incase of a bulk inserting , a node might overwrite another node
*         due to a push. record the overwritten node.
*
* @note   APPLICABLE DEVICES:      xCat3; AC5.
* @note   NOT APPLICABLE DEVICES:  None.
*
* @param[in] trieNodePtr              - the overwriting node
* @param[in] oldTrieNodePtr           - the overwritten node.
*
* @retval GT_OK                    - if the push worked.
* @retval GT_OUT_OF_PP_MEM         - if we found no free place.
*/
...
/**
* @internal releaseOldNodeRecord function
* @endinternal
*
* @brief   incase of a bulk inserting , a node might have overwriten another node
*         and the record of the that node needs to be freed.
*
* @note   APPLICABLE DEVICES:      xCat3; AC5.
* @note   NOT APPLICABLE DEVICES:  None.
*
* @param[in] trieNodePtr              - the node with the record
*
* @retval GT_OK                    - if the push worked.
* @retval GT_OUT_OF_PP_MEM         - if we found no free place.
*/
...
/**
* @internal pushUpTrie function
* @endinternal
*
* @brief   push up the above fathers of this node, until you find a place ,or not.
*
* @note   APPLICABLE DEVICES:      xCat3; AC5.
* @note   NOT APPLICABLE DEVICES:  None.
*
...
* @param[in] trieNodePtr              - the trie node to start from.
* @param[in] originalTrieNodePtr      - the trie node which rule will be freed by this push
...
*
* @param[out] allocNodePtrPtr          - the rule that was freed from this push, if there was one.
* @param[out] isInvalidateNeededPtr    - to indicate the higher level whether the place it
*                                      got needs invalidting first.
* @param[out] oldNodePtrPtr            - if the place needs invalidating then there was a
...
)
{
    ...
    if (ret == GT_OK)
    {
        for (i = sonsStackIdx-1; i>= 0; i--)
        {
            ...
            {
                PRV_SHARED_IP_LPM_DIR_LPM_TCAM_PAT_TRIE_SRC_GLOBAL_VAR_GET(dbgTcamWrites)++;
...
/**
* @internal checkNonValidFatherForNewSonsWithNotAllocRule function
* @endinternal
*
* @brief   check whether this node is non-valid father having "new" sons with
*         non-allocated rules
*
* @note   APPLICABLE DEVICES:      xCat3; AC5.
* @note   NOT APPLICABLE DEVICES:  None.
...
* @param[in] ipTcamShadowPtr          - the ip shadow we are working on.
*
* @param[out] isNewSonExixstPtr        - GT_TRUE: if there is any sone with non-allocated rooles.
*                                      - GT_FALSE: otherwise.
*
...
/**
* @internal pushDownTrie function
* @endinternal
*
* @brief   push down the below bounding sons of this node, until you find a place
*         ,or not.
*
* @note   APPLICABLE DEVICES:      xCat3; AC5.
* @note   NOT APPLICABLE DEVICES:  None.
...
* @param[in] entryType                - entry type
* @param[in] trieNodePtr              - the trie node to start from.
* @param[in] trieDestNodePtr          - trie node that allocation rule is making for
* @param[in] ipTcamFuncsPtr           - the utility functions to use for tcam access
* @param[in] isBulkInsert             - the type of insert this is: Bulk or regular.
* @param[in] defragEnable             - whether to defrag entries in case there is no
*                                      free place for this entry cause of entries
...
/**
* @internal ipPatTriePush function
* @endinternal
*
* @brief   allocate a place for the given trie node.
*
* @param[in] vrId                     - the virtual router id.
* @param[in] ipTcamShadowPtr          - the ip shadow we are working on.
* @param[in] entryType                - entry type
* @param[in] boundingSonPtr           - the sons which bounds this node from below.
* @param[in] validFatherPtr           - valid father of trieNode
* @param[in] ipTcamFuncsPtr           - the utility functions to use for tcam access
* @param[in] isBulkInsert             - the type of insert this is: Bulk or regular.
* @param[in] isMoveOnly               - if GT_TRUE only prefix moving is done
* @param[in] defragEnable             - whether to defrag entries in case there is no
*                                      free place for this entry cause of entries
*                                      fragmentation; relevant only if the TCAM entries
*                                      handler was created with partitionEnable = GT_FALSE
*
* @param[out] trieNodePtr              - the rule node of this trie node is updated
* @param[out] isInvalidateNeededPtr    - to indicate the higher level whether the place it
*                                      got needs invalidting first.
* @param[out] oldNodePtrPtr            - if the place needs invalidating then there was a
*                                      prefix there, this is the pointer for the prefix
*                                      node.
*
* @retval GT_OK                    - if the allocation worked.
* @retval GT_OUT_OF_PP_MEM         - if we found no free place.
* @retval GT_FAIL                  - if fatal error
*/
...
/**
* @internal ipPatTrieNodeAllocRule function
* @endinternal
*
* @brief   allocate a place for the given trie node.
*
...
* @param[in] ipTcamShadowPtr          - the ip shadow we are working on.
* @param[in] entryType                - entry type
* @param[in,out] trieNodePtr              - the trie node to start from.
* @param[in] boundingSonPtr           - the sons which bounds this node from below.
* @param[in] allocMethod              - the allocation method to use.
* @param[in] ruleNodePtr              - if not NULL then this is the rule to allocate (HSU use)
* @param[in] isBulkInsert             - the type of insert this is: Bulk or regular.
* @param[in] ipTcamFuncsPtr           - the utility functions to use for tcam access
* @param[in] isMoveOnly               - if GT_TRUE only prefix moving is done
* @param[in] defragEnable             - whether to defrag entries in case there is no
...
)
{
    ...
    PRV_CPSS_DXCH_LPM_TCAM_PAT_TRIE_NODE_STC *newRootTrie = NULL;
    PRV_CPSS_DXCH_LPM_TCAM_PAT_TRIE_NODE_STC *newKeyTrie = NULL;
    ...
    PRV_CPSS_DXCH_LPM_TCAM_PAT_TRIE_NODE_STC *currTrieNode = NULL;
    GT_PTR tcamRulePtr = NULL;
    GT_VOID *memAllocArray[PRV_CPSS_DXCH_LPM_TCAM_PAT_TRIE_MAX_PREFIX_LEN_IN_BYTES_CNS] = {NULL};
    GT_U32 numberOfAllocatedElements = 0;
    ...
    if (rootPtr->size == 0)
    {
        ...
        {
            if(rootPtr->tcamAllocInfo.tokenNode == NULL)
            {
                CPSS_LOG_ERROR_AND_RETURN_MAC(GT_OUT_OF_CPU_MEM, LOG_ERROR_NO_MSG);
            }
            memAllocArray[numberOfAllocatedElements] = rootPtr->tcamAllocInfo.tokenNode;
            numberOfAllocatedElements++;
        }
        ...
        else
        {
            ...
            else
            {
                ...
                if (ret != GT_OK)
                {
                    for (j = numberOfAllocatedElements-1; j >= 0; j--)
                    {
                        cpssOsFree(memAllocArray[j]);
                    }
    ...
    while (currTrieNode!= NULL)
    {
        ...
        if (i < currTrieNode->prefixLen) /* Split node*/
        {
            ...
            memAllocArray[numberOfAllocatedElements] = newRootTrie;
            numberOfAllocatedElements++;
            ...
            if (i < currPrefixLen)
            {
                ...
                memAllocArray[numberOfAllocatedElements] = newKeyTrie;
                numberOfAllocatedElements++;
                ...
                PRV_CPSS_DXCH_LPM_TCAM_ALLOCATE_TOKEN_NODE_MAC(ipTcamShadowPtr->tcamManagementMode,newKeyTrie);
                if(ipTcamShadowPtr->tcamManagementMode == PRV_CPSS_DXCH_LPM_TCAM_MANAGEMENT_TCAM_MANAGER_MODE_E)
                {
                    if(newKeyTrie->tcamAllocInfo.tokenNode == NULL)
                    {
                        for (j = numberOfAllocatedElements-1; j >= 0; j--)
                        {
                            cpssOsFree(memAllocArray[j]);
                    ...
                    memAllocArray[numberOfAllocatedElements] = newKeyTrie->tcamAllocInfo.tokenNode;
                    numberOfAllocatedElements++;
                }
            ...
            else
            {
                ...
                if(ipTcamShadowPtr->tcamManagementMode == PRV_CPSS_DXCH_LPM_TCAM_MANAGEMENT_TCAM_MANAGER_MODE_E)
                {
                    if(newRootTrie->tcamAllocInfo.tokenNode == NULL)
                    {
                        for (j = numberOfAllocatedElements-1; j >= 0; j--)
                        {
                            cpssOsFree(memAllocArray[j]);
                    ...
                    memAllocArray[numberOfAllocatedElements] = newRootTrie->tcamAllocInfo.tokenNode;
                    numberOfAllocatedElements++;
                }
        ...
        else
        {
                ...
                ((isEcmpPrefixSearch == GT_TRUE) && (i == currPrefixLen)))
            {
                ...
                PRV_CPSS_DXCH_LPM_TCAM_ALLOCATE_TOKEN_NODE_MAC(ipTcamShadowPtr->tcamManagementMode, currTrieNode);
                if(ipTcamShadowPtr->tcamManagementMode == PRV_CPSS_DXCH_LPM_TCAM_MANAGEMENT_TCAM_MANAGER_MODE_E)
                {
                    if(currTrieNode->tcamAllocInfo.tokenNode == NULL)
                    {
                        for (j = numberOfAllocatedElements-1; j >= 0; j--)
                        {
                            cpssOsFree(memAllocArray[j]);
                    ...
                    memAllocArray[numberOfAllocatedElements] = currTrieNode->tcamAllocInfo.tokenNode;
                    numberOfAllocatedElements++;
                }
...
/**
* @internal ipPatTrieTraverse function
* @endinternal
*
* @brief   a general pat trie DFS traverse function.
*
* @note   APPLICABLE DEVICES:      xCat3; AC5.
* @note   NOT APPLICABLE DEVICES:  None.
*
...
* @param[in] vrId                     - the virtual router id.
* @param[in] rootPtr                  - Root of the trie to search for the entry.
* @param[in] activateOnValidOnly      - whether to preform the given functions on valid
*                                      nodes only , or on all nodes.
* @param[in] downTraverseFunc         - the function to preform on the nodes on the way
*                                      down. (can be NULL for no function )
* @param[in] upTraverseFunc           - the function to preform on the nodes on the way
*                                      up. (can be NULL for no function )
* @param[in] invalidateTraverseFunc   - the function to preform on the nodes on
*                                      invalidation (can be NULL for no function )
* @param[in] extraData                - extra data to trasfer to the functions.
*                                      devListPtr          - pointer to the device list to preform the actions
*                                      on.
* @param[in] preformOnRoot            - whether to preform the actions on the root node.
* @param[in] isUpdateOnly             - is this an update only or a whole trie action.
*                                       GT_OK
*/
...
/**
* @internal ipPatTrieTraverseWriteFun function
* @endinternal
*
* @brief   a traverse write function
*
* @note   APPLICABLE DEVICES:      xCat3; AC5.
* @note   NOT APPLICABLE DEVICES:  None.
*
...
* @param[in] trieNodePtr              - the trie node that we are currently at.
* @param[in] extraData                - extra passed data
...
/**
* @internal ipPatTrieTraverseInvalidateFun function
* @endinternal
*
* @brief   a traverse invalidate function
*
* @note   APPLICABLE DEVICES:      xCat3; AC5.
* @note   NOT APPLICABLE DEVICES:  None.
*
...
/**
* @internal ipPatTrieTraverseDelFun function
* @endinternal
*
* @brief   a traverse del function
*
* @note   APPLICABLE DEVICES:      xCat3; AC5.
* @note   NOT APPLICABLE DEVICES:  None.
*
...
/**
* @internal ipPatTrieTraverseFreeFun function
* @endinternal
*
* @brief   a traverse free function
*
* @note   APPLICABLE DEVICES:      xCat3; AC5.
* @note   NOT APPLICABLE DEVICES:  None.
*
...
)
{
    ...
    if(ipTcamShadowPtr->tcamManagementMode == PRV_CPSS_DXCH_LPM_TCAM_MANAGEMENT_TCAM_MANAGER_MODE_E)
    {
        if(prefixBeforeRelocation.tcamAllocInfo.tokenNode == NULL)
        {
            CPSS_LOG_ERROR_AND_RETURN_MAC(GT_OUT_OF_CPU_MEM, LOG_ERROR_NO_MSG);
        }
    }
