...
)
{
    ...
    GT_VOID                                         *rollBackFreeArray[4] = {NULL};
    ...
    GT_BOOL                                         groupInsertOk = GT_FALSE;
    ...
    for(i=0; i <  ipTcamShadowPtr->shadowDevList.ipShareDevNum; i++)
    {
        ...
        if (ipLttEntryPtr->routeEntryBaseMemAddr >= routerNextHop)
            CPSS_LOG_ERROR_AND_RETURN_MAC(GT_BAD_PARAM, LOG_ERROR_NO_MSG);
    }


    lclIpGroup = (ipGroupPrefix == 0)?
        PRV_SHARED_IP_LPM_DIR_LPM_TCAM_COMMON_SRC_GLOBAL_VAR_GET(cpssDxChDefaultMcIpAddr)[protocolStack] : ipGroupPtr;

    lclIpGroupPrefix = (ipGroupPrefix == 0)?
        &(PRV_SHARED_IP_LPM_DIR_LPM_TCAM_COMMON_SRC_GLOBAL_VAR_GET(cpssDxChDefaultMcIpPrefixLen)[protocolStack]) : &ipGroupPrefix;
    ...
    if(pGroupNode == NULL)
    {
        ...
        cpssOsMemSet(mcGroupNode, 0, sizeof(PRV_CPSS_DXCH_LPM_TCAM_MC_GROUP_SRC_NODE_STC));
        ...
        if (ipSrcPrefix != 0)
        {
            ...
            if (prvCpssDxChLpmTcamPatTrieSearch(pRootTrie,
                                                PRV_SHARED_IP_LPM_DIR_LPM_TCAM_COMMON_SRC_GLOBAL_VAR_GET(cpssDxChDefaultMcIpAddr)[protocolStack],
                                                PRV_SHARED_IP_LPM_DIR_LPM_TCAM_COMMON_SRC_GLOBAL_VAR_GET(cpssDxChDefaultMcIpPrefixLen)[protocolStack],
    ...
    if (lclIpLttEntryPtr == NULL)
    {
        ...
        rollBackFreeArray[rollBackIndex] = lclIpLttEntryPtr;
        rollBackIndex++;
    }
    ...
    if ((ret == GT_OK) && (ipSrcPrefix != 0))
    {
        ...
        if (ret == GT_OK)
        {
            ...
            {
                /* src insertion is failed. Check if it was new group */
                if (pGroupNode == NULL)
                {
                    /* this is new group. It should be deleted */

                    ret2 = prvCpssDxChLpmTcamDeleteIpMcEntry(vrId,lclIpGroup,*lclIpGroupPrefix,ipSrcPtr, 0,protocolStack,ipTcamShadowPtr);
                    if (ret2 != GT_OK)
                    {
                        CPSS_LOG_ERROR_AND_RETURN_MAC(GT_FAIL, LOG_ERROR_NO_MSG);
                    }
    ...
    if (ret != GT_OK)
    {
        if ((pGroupNode == NULL) && (groupInsertOk == GT_FALSE))
        {
            cpssOsFree(mcGroupNode);
            if (isSrcTrieBeenCreated)
                prvCpssDxChLpmTcamPatTrieDestroy(ipTcamShadowPtr,pSrcTrie);
                                                     ...
                                                     (GT_PTR)&mcGroupNodePtr)) != NULL)
            {
                ...
                if (mcGroupNodePtr->isAPointer2Def == GT_TRUE)
                {
                    ...
                    if (ipTcamShadowPtr->tcamManagementMode == PRV_CPSS_DXCH_LPM_TCAM_MANAGEMENT_TCAM_ENTRIES_HANDLER_MODE_E)
                    {
                        ...
                        mcGroupRow    = mcGroupNodePtr->gIndex % PRV_SHARED_IP_LPM_DIR_LPM_TCAM_COMMON_SRC_GLOBAL_VAR_GET(tcamRows);
                        mcGroupColumn = mcGroupNodePtr->gIndex / PRV_SHARED_IP_LPM_DIR_LPM_TCAM_COMMON_SRC_GLOBAL_VAR_GET(tcamRows);
                    }
                    else
                    {
    ...
    if (ipSrcPrefix == 0)
    {
            ...
            (prvCpssDxChLpmTcamPatTrieIsEmpty(mcGroupNode->ipMcSrcTrie) == GT_FALSE))
        {
            ...
            case CPSS_IP_PROTOCOL_IPV4_E:
                defaultPrefixLen = PRV_SHARED_IP_LPM_DIR_LPM_TCAM_COMMON_SRC_GLOBAL_VAR_GET(cpssDxChDefaultMcIpPrefixLen)[CPSS_IP_PROTOCOL_IPV4_E];
                break;
            case CPSS_IP_PROTOCOL_IPV6_E:
                defaultPrefixLen = PRV_SHARED_IP_LPM_DIR_LPM_TCAM_COMMON_SRC_GLOBAL_VAR_GET(cpssDxChDefaultMcIpPrefixLen)[CPSS_IP_PROTOCOL_IPV6_E];
                break;
            default:
                CPSS_LOG_ERROR_AND_RETURN_MAC(GT_BAD_PARAM, LOG_ERROR_NO_MSG);
            }
...
/**
* @internal prvCpssDxchIpTcamMcGroupSrcNodeStcFreeFunc function
* @endinternal
*
           ...
           != NULL)
    {
        ...
        if(mcGroupNode->ipMcSrcTrie != NULL)
        {
            ...
            ret = prvCpssDxChLpmTcamPatTrieFlush(mcGroupNode->ipMcSrcTrie,
                                                 ...
                                                 srcEntryType,
                                                 ipMcTcamFuncs,
                                                 (GT_MEMFREEFUNCPTR)prvCpssDxchIpTcamMcGroupSrcNodeStcFreeFunc,
                                                 GT_TRUE);
...
)
{
    ...
    lclIpGroupPrefix = (ipGroupPrefixLen == 0)?
        &(PRV_SHARED_IP_LPM_DIR_LPM_TCAM_COMMON_SRC_GLOBAL_VAR_GET(cpssDxChDefaultMcIpPrefixLen)[protocolStack]) : &ipGroupPrefixLen;
...
)
{
    ...
    lclIpGroup = (ipGroupPrefix == 0)?
        PRV_SHARED_IP_LPM_DIR_LPM_TCAM_COMMON_SRC_GLOBAL_VAR_GET(cpssDxChDefaultMcIpAddr)[protocolStack] : ipGroupArr;
...
/**
* @internal ipMcTcamTrieUpdateData function
* @endinternal
*
...
/**
* @internal ipMcTcamTrieSetPrefixIp function
* @endinternal
*
...
/**
* @internal ipMcTcamTrieSetPrefixIpv4 function
* @endinternal
*
* @brief   utility function to Add IP UC prefix.
*
...
/**
* @internal ipMcTcamTrieSetPrefixIpv6 function
* @endinternal
*
* @brief   utility function to Add IP UC prefix.
*
...
/**
* @internal ipMcTcamTrieMovePrefix function
* @endinternal
*
...
* @retval GT_FAIL                  - on error
*
* @note In case of moving a MC group, then if there are sources associated with
    ...
    if ((srcTrieRootPtr != NULL) && (numOfSources != 0))
    {
        ...
        while (isLastNode == GT_FALSE)
        {
            ...
            if (nodePtr->pData != NULL)
            {
                ...
                {
                    if (srcTrieArray[i].tcamAllocInfo.tokenNode == NULL)
                    {
                        cpssOsFree(srcTrieArray);
                        CPSS_LOG_ERROR_AND_RETURN_MAC(GT_OUT_OF_CPU_MEM, LOG_ERROR_NO_MSG);
                    }
...
/**
* @internal ipMcTcamTrieMoveIpv4Prefix function
* @endinternal
*
* @brief   utility function to Update move rule entry in IP TCAM table.
*
...
/**
* @internal ipMcTcamTrieMoveIpv6Prefix function
* @endinternal
*
* @brief   utility function to Update move rule entry in IP TCAM table.
*
