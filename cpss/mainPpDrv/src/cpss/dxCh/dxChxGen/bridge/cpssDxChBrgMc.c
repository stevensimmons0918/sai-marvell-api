...
#define PRV_CPSS_DXCH_VIDX_NUM_CHECK_MAC(_devNum,_index)                                \
    ...
    {                                                                                   \
        if((_index) >=                                                                  \
            PRV_CPSS_DXCH_PP_MAC(_devNum)->accessTableInfoPtr[CPSS_DXCH_TABLE_MULTICAST_E].maxNumOfEntries)\
            CPSS_LOG_ERROR_AND_RETURN_MAC(GT_BAD_PARAM, "vidx limited to max[%d] got[%d]",\
                PRV_CPSS_DXCH_PP_MAC(_devNum)->accessTableInfoPtr[CPSS_DXCH_TABLE_MULTICAST_E].maxNumOfEntries-1,_index);  \
    }                                                                                   \
    else                                                                                \
    {                                                                                   \
