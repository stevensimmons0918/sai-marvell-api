...
#define FLD_OFFi(STR,idx,fld) idx*sizeof(STR) + offsetof(STR,fld)
    ...
    static const PRV_CPSS_DRV_FLD_ARR_INIT_STC prv_bc2PortRes_revA0[] =
    {
        /*--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
...
static const PRV_CPSS_DRV_FLD_ARR_INIT_STC prv_bc2PortRes_revB0[] =
{
    /*--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
...
/**
* @internal prvCpssDxChPortBcat2ResourcesRelease function
* @endinternal
*
...
)
{
    ...
    GT_U32 i;
    CPSS_DXCH_BCAT2_PORT_RESOURCE_LIST_STC       * resConfigPtr;

    rc = prvCpssArrDrvQeueryInit(&query,
                                 PRV_SHARED_PORT_PA_GLOBAL_VAR_GET(prv_dxChBcat2B0Ptr),
                                 devNum,0,(GT_U32*)PRV_CPSS_DXCH_DEV_REGS_VER1_MAC(devNum));
    if (rc != GT_OK)
    {
        return rc;
    }

    resConfigPtr = PRV_SHARED_PORT_PA_GLOBAL_VAR_GET(prv_bc2_B0_resConfigPtr);

    for (i = 0 ; i < sizeof(bc2_B0_resConfigPtr->arr)/sizeof(bc2_B0_resConfigPtr->arr[0]); i++)
    {
        if (resConfigPtr->arr[i].fldId != BC2_PORT_FLD_INVALID_E)
        {
...
)
{
    ...
    rc = prvCpssDxChBcat2_B0_Port_ResourcesCompute(devNum,portNum,ifMode,speed,coreClock,/*OUT*/resConfigPtr);
    if (rc != GT_OK)
    {
        return rc;
    }
    ...
    if (portMapShadowPtr->portMap.trafficManagerEn == GT_FALSE)
    {
        ...
        PRV_DXCH_BCAT2_RESOURCE_LIST_Get(resConfigPtr, BC2_PORT_FLD_TXDMA_SCDMA_TxQDescriptorCredit,          /*OUT*/&txQDescrCredits     );
        PRV_DXCH_BCAT2_RESOURCE_LIST_Get(resConfigPtr, BC2_PORT_FLD_TXDMA_SCDMA_TxFIFOHeaderCreditThreshold,  /*OUT*/&txFifoHeaderCredits );
        PRV_DXCH_BCAT2_RESOURCE_LIST_Get(resConfigPtr, BC2_PORT_FLD_TXDMA_SCDMA_TxFIFOPayloadCreditThreshold, /*OUT*/&txFifoPayloadCredits);
    }
    ...
    {
        rc = prvCpssDxChPort_Bcat2B0_ResourcesInHwConfig(devNum, resConfigPtr);
        if (rc != GT_OK)
        {
            return rc;
        }
    ...
    /* update DB    */
    prvCpssDxChBcat2_B0_PortResources_SpeedIdxGet(speed,/*OUT*/&speedMbps);
    rc = prvCpssDxChPortResourcesConfigDbCoreOverallSpeedAdd(devNum,0,speedMbps);
    if (rc != GT_OK)
    {
        return rc;
    }
...
)
{
    ...
    const CPSS_DXCH_BC2_PORT_RESOURCES_FLD_ENT res2freeList[] =
    {
    ...
    for (i = 0 ; i < sizeof(res2freeList)/sizeof(res2freeList[0]); i++)
    {
        ...
        /* array bound check */
        if ((GT_U32)resId >= BC2_PORT_FLD_MAX)
        {
            CPSS_LOG_ERROR_AND_RETURN_MAC(GT_FAIL, LOG_ERROR_NO_MSG);
        }
        if (resConfigPtr->arr[resId].fldId != BC2_PORT_FLD_INVALID_E)
        {
            resConfigPtr->arr[resId].fldVal = 0;
        }
    }
    rc = prvCpssDxChPort_Bcat2B0_ResourcesInHwConfig(devNum, resConfigPtr);
    if (rc != GT_OK)
    ...
    /* update DB */
    prvCpssDxChBcat2_B0_PortResources_SpeedIdxGet(speed,/*OUT*/&speedMbps);
    rc = prvCpssDxChPortResourcesConfigDbCoreOverallSpeedDelete(devNum,0,speedMbps);
    if (rc != GT_OK)
    {
        return rc;
    }
...
)
{
    ...
    CPSS_DXCH_BCAT2_PORT_RESOURCE_LIST_STC        old_bc2_B0_TM_resConfig;
    CPSS_DXCH_BCAT2_PORT_RESOURCE_LIST_STC       * resConfigPtr;
    ...
    rc = prvCpssDxChBcat2_B0_TM_ResourcesCompute(devNum,PRV_CPSS_PP_MAC(devNum)->coreClock,tmBandwidthMbps,resConfigPtr);
    if (rc != GT_OK)
    {
        return rc;
    }
...
)
{
    ...
    rc = prvCpssArrDrvQeueryInit(&query, PRV_SHARED_PORT_PA_GLOBAL_VAR_GET(prv_dxChBcat2B0Ptr),
                                  devNum, 0,(GT_U32*)PRV_CPSS_DXCH_DEV_REGS_VER1_MAC(devNum));
    if (rc != GT_OK)
    {
        return rc;
    ...
    rc = prvCpssArrDrvFldAttributeArrSize(PRV_SHARED_PORT_PA_GLOBAL_VAR_GET(prv_dxChBcat2B0Ptr),
                                          resourceId, &size);
    if (rc != GT_OK)
    {
        return rc;
...
/**
* @internal prvCpssDxChPortBcat2B0ResourceHwInit function
* @endinternal
*
* @brief   Initialize HW with port resource
*
* @note   APPLICABLE DEVICES:      Bobcat2; Caelum; Aldrin; AC3X; Bobcat3; Aldrin2; Falcon; AC5P; AC5X; Harrier; Ironman.
* @note   NOT APPLICABLE DEVICES:  xCat3; AC5; Lion2.
*
...
/**
* @internal prvCpssDxChBcat2PortResourcesHwInit function
* @endinternal
*
* @brief   Initialize HW with port resources
*
* @note   APPLICABLE DEVICES:      Bobcat2; Caelum; Aldrin; AC3X; Bobcat3; Aldrin2; Falcon; AC5P; AC5X; Harrier; Ironman.
* @note   NOT APPLICABLE DEVICES:  xCat3; AC5; Lion2.
*
...
)
{
    ...
    if(PRV_CPSS_SIP_5_CHECK_MAC(devNum))
    {
        ...
        if (!PRV_SHARED_PORT_PA_GLOBAL_VAR_GET(prv_dxChBcat2B0Ptr))
        {
            PRV_SHARED_PORT_PA_GLOBAL_VAR_GET(prv_dxChBcat2B0Ptr) = cpssOsMalloc(sizeof(PRV_CPSS_DXCH_ARR_DRV_STC));
            if (!PRV_SHARED_PORT_PA_GLOBAL_VAR_GET(prv_dxChBcat2B0Ptr))
            {
                CPSS_LOG_ERROR_AND_RETURN_MAC(GT_OUT_OF_CPU_MEM, LOG_ERROR_NO_MSG);
            }

            PRV_SHARED_PORT_PA_GLOBAL_VAR_GET(prv_dxChBcat2B0_FldInitStcArr) =
                cpssOsMalloc((BC2_PORT_FLD_MAX) * sizeof(PRV_CPSS_DRV_FLD_ARR_DEF_STC));

            if (!PRV_SHARED_PORT_PA_GLOBAL_VAR_GET(prv_dxChBcat2B0_FldInitStcArr))
            {
                cpssOsFree(PRV_SHARED_PORT_PA_GLOBAL_VAR_GET(prv_dxChBcat2B0Ptr));
                CPSS_LOG_ERROR_AND_RETURN_MAC(GT_OUT_OF_CPU_MEM, LOG_ERROR_NO_MSG);
            }

            PRV_SHARED_PORT_PA_GLOBAL_VAR_GET(prv_bc2_B0_resConfigPtr) =
                cpssOsMalloc(sizeof(CPSS_DXCH_BCAT2_PORT_RESOURCE_LIST_STC));

            if (!PRV_SHARED_PORT_PA_GLOBAL_VAR_GET(prv_bc2_B0_resConfigPtr))
            {
                cpssOsFree(PRV_SHARED_PORT_PA_GLOBAL_VAR_GET(prv_dxChBcat2B0Ptr));
                cpssOsFree(PRV_SHARED_PORT_PA_GLOBAL_VAR_GET(prv_dxChBcat2B0_FldInitStcArr));
                CPSS_LOG_ERROR_AND_RETURN_MAC(GT_OUT_OF_CPU_MEM, LOG_ERROR_NO_MSG);
            }
        }

        intSeqPtr = &prv_bc2PortRes_revB0[0];
        rc = prvCpssArrDrvInit(/*INOUT*/PRV_SHARED_PORT_PA_GLOBAL_VAR_GET(prv_dxChBcat2B0Ptr)
                                        ,PRV_SHARED_PORT_PA_GLOBAL_VAR_GET(prv_dxChBcat2B0_FldInitStcArr)
                                        ,intSeqPtr
                                        ,BC2_PORT_FLD_MAX);
        if (rc != GT_OK)
...
)
{
    ...
    PRV_CPSS_DXCH_DEV_CHECK_MAC(devNum);
    PRV_CPSS_NOT_APPLICABLE_DEV_CHECK_MAC(devNum, CPSS_XCAT3_E | CPSS_AC5_E | CPSS_LION2_E);
    CPSS_NULL_PTR_CHECK_MAC(resourcePtr);
    cpssOsMemSet(resourcePtr, 0, sizeof(PRV_CPSS_DXCH_BC2_PORT_RESOURCE_STC));
    ...
    rc = prvCpssArrDrvQeueryFldGet(&query, BC2_PORT_FLD_TXDMA_SCDMA_TxQDescriptorCredit, portMapShadowPtr->portMap.txDmaNum, /*OUT*/&val);
    if (rc != GT_OK)
    {
        return rc;
    }
    resourcePtr->txdmaTxQCreditValue = val;

    rc = prvCpssArrDrvQeueryFldGet(&query, BC2_PORT_FLD_TXDMA_SCDMA_TxFIFOHeaderCreditThreshold, portMapShadowPtr->portMap.txDmaNum, /*OUT*/&val);
    if (rc != GT_OK)
    {
        return rc;
    }
    resourcePtr->txdmaTxfifoHeaderCounterThresholdScdma = val;


    rc = prvCpssArrDrvQeueryFldGet(&query, BC2_PORT_FLD_TXDMA_SCDMA_TxFIFOPayloadCreditThreshold, portMapShadowPtr->portMap.txDmaNum, /*OUT*/&val);
    if (rc != GT_OK)
    {
        return rc;
    }
    resourcePtr->txdmaTxfifoPayloadCounterThresholdScdma = val;
    return GT_OK;
}
