...
if __name__ == '__main__':
    ...
    opts, files = getopt.getopt(sys.argv[1:], "?o:x:DS:z:E:FI:X:q")
    for (opt,val) in opts:
        if opt == "-o":
            outfile_name = val
        elif opt == "-x":
    ...
    def open_output(name):
        ...
        if size_limit != None:
            ...
            print >>of,"#include <cmdShell/FS/cmdFS_RAM.h>"
            print >>of, ""
        cont = None
        lst = list()
        listing = ""
    ...
    def output_file_flush(next_c = None,next_chunk_follow = None):
        ...
        for i in lst:
            print >>of, "    inode = cmdFS_RAM_AddEmbeddedFileEntry(\"%s\", %d, %dUL);"%(
                        i["filename"].encode("string-escape"), i["datasize"], i["mtime"])
