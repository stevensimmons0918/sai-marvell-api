...
_reSignatureParams = re.compile(r"^\s*((?:IN|OUT|INOUT)(?:\s*const)?)?\s+([_a-zA-Z0-9]+)\s+(\*{0,2}\s*[_a-zA-Z0-9]+(?:\[[^\]]*\])?)\s*(?:\,*.+=([_a-zA-Z0-9]+)\*)?", re.M )
...
_reCommentedTypedef = [
    re.compile(\
                ...
                re.M | re.DOTALL),
    re.compile(\
                r"(/\*[^*]*\*+(?:[^/*][^*]*\*+)*/)\s*typedef\s+(enum|struct|union)()\s*\{",\
                re.M | re.DOTALL)
]
...
def scan_struct_descriptions(source):
    ...
    while True:
        m = None
        for r in _reCommentedTypedef:
            m1 = r.search(source, pos);
            if m1 != None:
                if m == None:
                    m = m1
                elif m1.start(0) < m.start(0):
                    m = m1
        if m == None:
            break
        p = m.end(0)
