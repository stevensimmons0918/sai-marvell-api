            ...
            lua_getfield(L,-1,"cpu");
                ...
                if (0 != strcmp(token,"none"))
                {
                    ...
                    if (0 == strcmp(token,"fixed"))
                    {
                        ...
                        /* compiler required bounds checking - should always pass */
                        if ((sliceNum <= CPSS_PA_NUMBER_OF_CPU_SLICES) && (sliceNum != -1))
                        {
...
{
    GT_U32 id, size0, size1;
    int len;
    portPriorityMatrPtr->portN = total_ports;

    lua_getfield(L,-1,"port_priority");
    id = 0;
    size0 = sizeof(portPriorityMatrPtr->portPriorityList[0]) / sizeof(portPriorityMatrPtr->portPriorityList[0][0]);
    size1 = sizeof(portPriorityMatrPtr->portPriorityList) / sizeof(portPriorityMatrPtr->portPriorityList[0]);
    lua_pushnil(L);  /* first key */
    while (lua_next(L, -2) != 0) 
    {
        if (id >= size1)
        ...
        len = luaTop_ArrU8Get_woName(L, size0, /*OUT*/ portPriorityMatrPtr->portPriorityList[id]);
        if ((len < 0) || ((GT_U32)len >= size0))
        {
