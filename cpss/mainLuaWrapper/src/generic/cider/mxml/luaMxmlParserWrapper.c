...
)
{
    ...
    for (node = parent->child; node; )
    {
        ...
        for (i = 0; i < node->value.element.num_attrs; i++)
        {
            mxml_attr_t *attr = node->value.element.attrs + i;
            if (_mxml_strcmp(attr->name, attrname) == 0 &&
                prvLuaXml_streqx(attr->value, prefix, (int)prefix_len, ignorecase))
            {
...
static int prvLuaXmlOpen(lua_State *L)
{
    ...
    fp->fd = fs->open(filename, GEN_FS_O_RDONLY);
    if ((int)fp->fd < 0)
    {
        lua_pushnil(L);
        return 1;
    }
    root = prvCpssMxmlLoadFd(NULL, fp, fs, MXML_NO_CALLBACK);
    fs->close(fp->fd);
    if (!root)
    {
        lua_pushnil(L);
...
static int prvLuaXmlNewXML(lua_State *L)
{
    ...
    node = prvCpssMxmlNewXML(lua_tostring(L,1));
    if (node)
        lua_pushlightuserdata(L, node);
    else
        lua_pushnil(L);
...
static int prvLuaXmlNewElement(lua_State *L)
{
    ...
    node = prvCpssMxmlNewElement((mxml_node_t *)lua_touserdata(L,1), lua_tostring(L,2));
    ...
    else
    {
        if (lua_isstring(L,3))
        {
             prvCpssMxmlNewText(node, 0, lua_tostring(L,3));
        }
    }
    lua_pushlightuserdata(L, node);
...
static int prvLuaXmlSaveXmlFile(lua_State *L)
{
    ...
    fp->fd = fs->open(lua_tostring(L,2) ,GEN_FS_O_RDWR | GEN_FS_O_CREAT);   
    prvCpssMxmlSaveFd((mxml_node_t *)lua_touserdata(L,1), fp, fs, prvLuaXmlNewLineCallback);
    fs->close(fp->fd); 
    cpssOsFree(fp);
    lua_pushinteger(L,0);
    return 1;
...
static int prvLuaXmlAddAttrib(lua_State *L)
{
    ...
    prvCpssMxmlElementSetAttr(lua_touserdata(L, 1),lua_tostring(L,2),lua_tostring(L,3));
    lua_pushinteger(L,0);
    return 1;
}
...
static int prvLuaXmlDelete(lua_State *L)
{
    ...
    prvCpssMxmlDelete((mxml_node_t *)lua_touserdata(L, 1));
    lua_pushinteger(L,0);
    return 1;
}
